...
| `APP_TOKEN` | (ตัวเลือก) shared token กันคนสุ่มเจอ URL — ต้องตรงกับ `APP_TOKEN` ใน `config.js` |

> 🔑 **APP_TOKEN (กันขั้นต่ำ):** ตั้งค่าเดียวกันทั้งใน Script Property `APP_TOKEN` และ `config.js`
> แล้ว backend จะปฏิเสธ request ที่ไม่มี token (ยกเว้น LINE webhook) — ถ้าปล่อยทั้งสองที่ว่าง = ปิดการตรวจ
> ⚠️ frontend เป็น static สาธารณะ ใครเปิด view-source ก็เห็น token ได้ จึงกันได้แค่คนสุ่มเจอ URL ไม่ใช่ security เต็มรูปแบบ
