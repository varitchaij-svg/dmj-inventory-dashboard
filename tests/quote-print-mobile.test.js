...
// ─────────────────────────────────────────────────────────────────────────────
// ปุ่ม "แก้ไข" ก็หายไปจากมือถือด้วย — คนละปุ่มกับปุ่มพิมพ์ ต้องคุมแยก
// ─────────────────────────────────────────────────────────────────────────────
// เจอตอนเทียบภาพหน้าจอแนวตั้ง/แนวนอนคู่กัน (2026-08-08): รอบที่แก้ปุ่มพิมพ์ข้างบนไล่ดูเฉพาะ
// "ปุ่มพิมพ์" จึงเก็บตกปุ่ม ✏️ แก้ไข ที่ตารางมีแต่การ์ดมือถือไม่มี — อาการเดียวกันเป๊ะ
// (ปุ่มหายเงียบ ๆ ไม่มี error) แต่เจ็บกว่าเพราะ **แก้ใบเสนอราคาจากมือถือไม่ได้เลย**
// ต้องหาคอมหรือหมุนจอ · คุมแยกเพราะเทสต์ชุดบนจับเฉพาะ handlePrint ไม่ครอบถึง handleEdit
describe('ปุ่มแก้ไขใบเสนอราคาบนมือถือ (โหมดรออนุมัติ)', () => {
  // แก้ไขได้เฉพาะใบที่ยัง "รออนุมัติ" — โหมดอนุมัติแล้วไม่มีปุ่มนี้ทั้ง 2 ฝั่งโดยตั้งใจ
  const PENDING_MOBILE = MOBILE_BLOCKS[0];

  it('การ์ดมือถือมีปุ่มแก้ไข (handleEdit) เหมือนตาราง', () => {
    expect(PENDING_MOBILE).toMatch(/handleEdit\(q\)/);
  });

  it('ปุ่มแก้ไขมีป้ายชื่อ ไม่ใช่ไอคอนดินสอเปล่า ๆ', () => {
    // ผู้ใช้หลักคือพนักงานที่อ่านรหัส/ไอคอนไม่คล่อง — ✏️ เดี่ยว ๆ เดาไม่ออกว่าทำอะไร
    expect(PENDING_MOBILE).toMatch(/✏️ แก้ไข/);
  });

  it('ประกาศ editingThis ก่อนใช้ (ไม่ประกาศ = ReferenceError → จอขาวทั้งแท็บ)', () => {
    expect(PENDING_MOBILE).toMatch(/const editingThis = editingId === \(q\.id \|\| q\.number\)/);
  });

  it('ระหว่างโหลดรายละเอียดใบเพื่อแก้ไข ปุ่มอื่นถูกล็อกด้วย (anyBusy รวม editingId)', () => {
    // ไม่รวม = กดแก้ไขแล้วยังกดอนุมัติ/ปิดใบซ้อนได้ระหว่างรอโหลด
    expect(PENDING_MOBILE).toMatch(/const anyBusy = [^\n]*!!editingId/);
  });

  it('ตารางจอกว้างยังมีปุ่มแก้ไขอยู่ (ไม่ถูกทำหายระหว่างแก้ฝั่งมือถือ)', () => {
    expect(DESKTOP_ONLY).toMatch(/handleEdit\(q\)/);
  });
});
