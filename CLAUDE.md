...
**มี Vitest test suite แล้ว** — 903 tests, 26 test files, ทั้งหมด pass
...
14. **ABC (A/B/C) มี 2 ที่ที่คำนวณแยกกัน** — `abcClassify` (views-analytics.jsx, คิว "ควรนับ/
    ควรเช็คก่อน") กับ ABC ในการ์ดหน้าภาพรวม (views-main.jsx) · **ทั้งคู่ต้องใช้ฐานเดียวกัน**:
    ยอดขาย 12 เดือนสมบูรณ์ล่าสุด (`abcWindowRev`) + ตัดคลาสด้วย cumulative **ก่อนบวกตัวเอง**
    ไม่งั้นสินค้าตัวเดียวกันเป็น A ที่หน้าหนึ่ง C อีกหน้าหนึ่ง (มี meta-test ล็อกไว้แล้ว)
15. **"เฉลี่ย N เดือนล่าสุด" ห้ามนับเดือนปัจจุบัน** — ใช้ `completeMonths(p.monthly)` เสมอ
...
16. **"มูลค่าสต๊อก" = ราคาขายส่ง ไม่ใช่ราคาป้าย** — GAS คูณ `WHOLESALE_RATIO`
...
17. **ZORT ไม่ให้ราคาทุนใน PO** (ยืนยัน `debugPurchasePrices()` 1 ส.ค. 2026: `pricepernumber`
...
18. **กำไรคิดจาก `soldRev` ไม่ใช่ `price` (ราคาป้าย)** — ร้านขายส่ง −20% เป็นเรื่องปกติ
