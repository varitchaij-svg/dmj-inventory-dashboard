...
- เทสต์: `tests/home-menu.test.js` (35 เคส — eval `groupTabsFor`/`splitTabLabel`/ตารางจริงจาก
  `app.jsx` ไม่ copy) + browser test 4 เคส (owner/warehouse/frontstore + โหมด `nodata=1`)
  ที่**นับจำนวนการ์ดเทียบกับ `ROLE_TABS` ตรง ๆ** — "เมนูหายไป 1 อัน" คือความพังที่หน้าจอยัง
  ดูปกติทุกประการ ไม่มีใครรายงานว่าเป็นบั๊ก
