...
4. **`r.json()` ตรง ๆ ยังหลงเหลืออยู่** ใน `fetchFromSheet`/`fetchOrdersOnly`/prefetch ใน `<head>`
   → ผู้ใช้เห็น `SyntaxError: Unexpected token '<'` · เปลี่ยนเป็น `dmjJson`/`dmjErrText` แล้ว
   (prefetch ทำซ้ำแบบย่อในไฟล์ HTML เพราะตอนนั้น `ui.jsx` ยังโหลดไม่เสร็จ)
   · ⚠️ **เส้นทางล็อกอินตกสำรวจทั้งเส้นในรอบนี้** — เพิ่งแก้ใน Phase 7.6 (ดูหัวข้อถัดไป)
5. `products[].locationRaw` **ไม่มีใครอ่านเลย** — เลิกส่ง (ไบต์เปล่าที่คูณด้วยจำนวน SKU ทุกตัว)

เทสต์: `tests/stampede.test.js` (55 เคส) — ข้อ 3 เป็น**เทสต์พฤติกรรมจริง** (eval `dmjFetch`
จาก `ui.jsx` แล้วให้ `fetch` คืน promise ที่ไม่มีวัน settle ยืนยันว่า abort ทำงาน) ไม่ใช่แค่จับ string

### Phase 7.6 — ล็อกอิน (ส.ค. 2026): Phase 7.5 ไม่ได้ครอบเส้นทางนี้เลยแม้แต่จุดเดียว

**รายงานจากผู้ใช้จริง (iPhone ทั้ง PWA และ Safari)**: ค้างหน้า "กำลังรอผลล็อกอิน" · วนกลับไป
หน้าล็อกอินซ้ำ · ขึ้นข้อความ error อ่านไม่ออก · ช้ามากแต่สุดท้ายเข้าได้ — **ทั้ง 4 อาการมีสาเหตุ
คนละตัวกัน** และทุกตัวอยู่ในเส้นทางที่ Phase 7.5 มองข้ามไป

**ต้นเหตุร่วม**: `postAuthAction` (app.jsx) = ทางผ่านของ **authLine / me / logout /
claimLoginHandoff ทั้งหมด** ยังใช้ `fetch` ดิบ + `res.json()` ดิบ · ตอนนี้ผ่าน `dmjFetch`+`dmjJson`
แล้ว พร้อมเพดานเวลาแยกตามงาน (`AUTH_TIMEOUT_MS` 25 วิ · `AUTH_ME_TIMEOUT_MS` 20 วิ ·
`AUTH_CLAIM_TIMEOUT_MS` 8 วิ) — **claim ต้องสั้นสุดเสมอ** เพราะถูกยิงซ้ำเป็นรอบ ๆ ไม่ใช่ one-shot

1. **จอ `checking` เคยเป็นทางตัน** — สปินเนอร์เปล่า ไม่มีข้อความ ไม่มีปุ่ม ไม่มี timeout และ
   `?code=` ถูก `history.replaceState` ล้างไปแล้ว → **รีเฟรชก็ไม่ช่วย ต้องปิดแอปเปิดใหม่**
   · ตอนนี้: นับวินาทีตลอด · 12 วิ บอก "ช้ากว่าปกติ" · **30 วิ มีปุ่ม "กลับไปหน้าล็อกอิน" เสมอ**
   ⚠️ ปุ่มนี้ต้องไม่พึ่ง timeout ของ fetch — เป็นตาข่ายชั้นสุดท้ายที่ต้องทำงานแม้ทุกอย่างค้าง
2. **poll handoff ยิงซ้อนกันเอง** — เดิม `setInterval` 4 วิ คงที่ **ไม่รอ tick ก่อนหน้า**
   GAS ตอบช้ากว่า 4 วิ เป็นเรื่องปกติ (cold start) → ยิ่งช้ายิ่งยิงถี่ = ทำร้ายตัวเอง
   · ตอนนี้ `setTimeout` เรียกตัวเอง + `busy` guard + ถอย 4→8→15 วิ
   · **`visibilitychange`/`focus` ห้ามถอด** (iOS แช่แข็ง timer — ทางกู้หลักของ PWA)
     ตอนตื่นรีเซ็ตระยะถอยกลับไปเร็วสุด
3. **TTL ไม่ตรงกัน 3 ที่** — client handoff 15 นาที แต่ client state 30 นาที และ server 15 นาที
   → คนที่ล็อกอินเสร็จนาทีที่ 16-30 ผ่านการเช็ค state ได้ **แต่ฝั่ง PWA เลิกตามผลไปแล้ว**
   · ตอนนี้ **30 นาทีทั้ง 3 ที่** (`LINE_STATE_TTL_MS` / `LINE_HANDOFF_TTL_MS` /
   `LOGIN_HANDOFF_TTL_SEC`) — แก้ที่เดียวไม่พอ ต้องแก้พร้อมกันเสมอ
4. **handoff หายถาวรเมื่อคำตอบไปไม่ถึงเครื่อง** — GAS `cache.remove()` **ก่อน** ส่งคำตอบ
   ถ้าก้อนถูกตัดกลางทาง (เน็ตร้านหลุด/ลิงก์หมดอายุ) token หายเลย ผู้ใช้ต้องล็อกอินใหม่ทั้งรอบ
   · ตอนนี้เขียนทับด้วยค่าเดิม TTL `LOGIN_HANDOFF_GRACE_SEC` (60 วิ) แทนการลบ — ยื่นซ้ำได้
   ในช่วงสั้น ๆ **ยังคงความเป็น "ใช้ครั้งเดียวต่อการล็อกอิน 1 รอบ"**
   · ฝั่ง client: `claimHandoff` เคยส่ง `d.staff` ที่เป็น `undefined` เข้า `applyStaffSession`
   → โยน TypeError แล้วถูกกลืนใน `catch` **เงียบสนิท** ทั้งที่ server ปล่อยผลออกมาแล้ว
5. **`resolveSession_` อ่านชีตเซสชันทั้งใบ + เขียน `lastSeenAt` ทุก request** — ถูกเรียกทุก
   doPost คูณกับ poll ทุก 4 วิ ของทุกเครื่องที่รอล็อกอิน = ตัวการของ "ล็อกอินช้ามาก"
   · ตอนนี้ cache ใน CacheService 5 นาที (คีย์ = แฮชของ token **ไม่เก็บ token ดิบ**) +
   เขียน `lastSeenAt` เฉพาะเมื่อค่าเดิมเก่ากว่า 10 นาที + `doPost` **ข้ามการ resolve ทั้งหมด**
   เมื่อ action อยู่ใน `SESSION_EXEMPT_ACTIONS_` (`claimLoginHandoff` คือตัวที่ถูกยิงถี่สุด)
   · ⚠️ **cache เก็บแค่ `staffId` ห้ามเก็บ staff ทั้งก้อน** — role/สถานะเปลี่ยนได้จากแท็บ
   "พนักงาน" ถ้า cache ทั้งก้อน เจ้าของถอดสิทธิ์ใครแล้วคนนั้นยังใช้ต่อได้อีก 5 นาที
   · ⚠️ **`revokeSession_` ต้องล้าง cache ด้วย** ไม่ใช่แค่ติ๊กในชีต — ไม่งั้นกด "ออกจากระบบ"
   แล้ว token ยังใช้ได้ต่อ (ช่องโหว่ที่ไม่มี error ให้เห็นเลย)
   · ห้าม cache ยาวเกินอายุที่เหลือของ session (จะกลายเป็นการยืดอายุให้เองโดยไม่ตั้งใจ)

**แถบความคืบหน้า** (ผู้ใช้ขอมาเอง — "อยากรู้ว่าใช้เวลาเข้านานไหม"):
- **จอโหลดข้อมูลหลัก = นับไบต์จริง** ผ่าน `dmjJsonProgress` (ui.jsx) ที่อ่าน `res.body.getReader()`
  · ตัวหารคือ **ขนาดจริงของครั้งล่าสุดที่สำเร็จ** (`localStorage.dmj_last_payload_bytes`)
  **ห้าม hard-code** — จำนวนสินค้าโตทุกเดือน ค่าที่ค้างไว้จะเพี้ยนขึ้นเรื่อย ๆ (บทเรียนเดียวกับ
  `soldQty/5`) · ตันที่ **95%** ไม่โชว์ 100% ทั้งที่ยังไม่จบ
- **จอล็อกอิน = นับวินาที** (คำตอบไม่กี่ร้อยไบต์ `%` ไม่มีความหมาย)
- ⚠️ **ห้ามทำแถบที่วิ่งตามเวลาแล้วอ้างเป็น %** — ผู้ใช้จะเชื่อว่าใกล้เสร็จทั้งที่อาจเพิ่งเริ่ม
  บนเน็ตร้านที่ช้าจริง นั่นทำให้ตัดสินใจผิด (ปิดแอปทิ้ง/กดซ้ำ)
- ⚠️ `dmjJsonProgress` **ส่ง text ที่อ่านมาแล้วกลับเข้า `dmjJson`** แทนการ parse เอง —
  error path ต้องเป็นตัวเดียวกันเป๊ะ (ข้อความไทย + `dmjKind='badjson'` + `dmj_last_backend_error`)
  ถ้าเขียนใหม่ให้ "คล้าย ๆ" มันจะ drift ออกจากกันโดยไม่มีอะไรเตือน
  · เบราว์เซอร์ไม่รองรับ stream → **ถอยไปใช้ `dmjJson` ทั้งดุ้น** ไม่ทำครึ่ง ๆ กลาง ๆ

เทสต์: `tests/login-resilience.test.js` (48 เคส) — ฝั่ง GAS **eval ฟังก์ชันจริงจาก `.gs`**
(เหมือน auth.test.js ไม่ copy) รันกับชีต/CacheService ปลอม ยืนยันพฤติกรรมจริงของ cache
ทั้งเรื่องความเร็ว (อ่านชีตครั้งเดียว) และความถูกต้อง (role เปลี่ยนแล้วเห็นทันที · revoke แล้วใช้ไม่ได้)
