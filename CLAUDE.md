...
#### 📊 นับสต็อก/เช็คหน้าร้าน = "รอบการนับ" ไม่ใช่ราย SKU (ส.ค. 2026)

เจ้าของทัก "เห็นพนักงานกดนับกันเยอะ แต่ตัวเลขในหน้าผลงานน้อย" · ต้นเหตุ: เดิม `confirmStockCount`
(นับคลัง) และ `updateFrontStore` (เช็คหน้าร้าน) เขียน audit **เฉพาะ SKU ที่ค่าเปลี่ยน** → นับ 100
ตัวตรงกับระบบ 85 ได้เครดิตแค่ 15 (**งานนับส่วนใหญ่หายเงียบ ๆ** — งานนับคือ "ตรวจ" ไม่ใช่ "เจอผิด")

- เจ้าของเลือก: **นับเป็น "รอบ" ไม่ใช่ราย SKU** (Audit ไม่บวม) · **1 การกดยืนยัน = 1 แถว** action เดิม
  (`"นับสต็อก"`/`"ตรวจหน้าร้าน"`) · resource = **`"รอบนับ:N"`** (N = นับทั้งรอบกี่ตัว) · detail เก็บ
  "ที่เปลี่ยนจริง" (capped 40) → ตามรอยได้ว่าใครแก้อะไร โดยไม่เขียน audit ราย SKU
- **`staffPerfRowUnits_(resource)`** ดึง N จาก `"รอบนับ:N"` (แถวเก่า/งานอื่น → 1, backward compat) ·
  `staffPerfAggregateAudit_` รวม **`byCatUnits`** (จำนวนตัว) คู่กับ `byCat` (จำนวนรอบ)
- category `count`/`fscheck`: `unit:"รอบ"` + **`sumUnits:true`** → UI โชว์ **"N รอบ · รวม M ตัว"**
- ⚠️ **`total`/`perHour` นับ 1 รอบ = 1 งาน** (ไม่ใช่ N ตัว) — งานนับ 1 รอบเทียบเท่า 1 ออเดอร์ในยอดรวม
  ปริมาณจริงดูที่ "รวม M ตัว" แทน · ⚠️ **`staffPerfRowUnits_` ห้ามดึงเลขจาก SKU** (`OL00008` มีเลข
  ในตัว) — match เฉพาะ prefix `"รอบนับ:"` ที่เราเขียนเอง
- ⚠️ **ห้ามกลับไป log ราย SKU เฉพาะที่เปลี่ยน** — จะได้เครดิตแค่ discrepancy อีก (มีเทสต์กัน)
- เทสต์: `tests/staff-perf.test.js` (+11 เคส — `staffPerfRowUnits_`/`staffPerfCountDetail_`/
  byCatUnits + meta-test ว่า 2 flow เขียน `"รอบนับ:"` และ category ใช้ `unit:"รอบ"`+`sumUnits`)
...
- **เทียบข้ามตำแหน่งไม่ได้** — นับสต็อก = 1 แถว/รอบ · ขาย = 1 แถว/บิล → UI **จัดกลุ่มตามตำแหน่ง**
