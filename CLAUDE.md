...
**6. กำไรคิดจาก "เงินที่เข้าจริง" ไม่ใช่ราคาป้าย** (MarginView)
`profit = soldRev − ต้นทุน × revQty` · `marginPct = profit / soldRev`
- เดิมใช้ `(p.price − cost) × soldQty` ซึ่ง `p.price` = **ราคาปลีก** แต่ร้านขายส่ง −20% เป็นปกติ
  → กำไรที่โชว์เกินจริงเท่ากับส่วนลดเสมอ · ตารางจึงโชว์ **"ขายจริง/ชิ้น"** (`soldRev ÷ revQty`)
  ไม่ใช่ราคาป้าย — ถ้าโชว์ราคาป้ายคู่กับ % ที่คิดจากราคาจริง ผู้ใช้กดคิดเลขตามแล้วไม่ตรง
- **`revQty = soldQty − soldQtyUnscanned`** — `p.soldQty` รวมของที่ถูกดึงไปทำงาน MTO
  (`soldQtyUnscanned`) แต่ `p.soldRev` **ไม่รวม** เงินก้อนนั้น (อยู่ในงาน MTO)
  ถ้าคิดต้นทุนทั้ง `soldQty` จะกลายเป็น "มีต้นทุนแต่ไม่มีรายได้" = **ขาดทุนปลอม**

**7. ฐานสินค้าของ "ยอดขาย" กับ "มูลค่าสต๊อก" คนละชุดโดยตั้งใจ**
...
**8. ABC ใช้หน้าต่าง 12 เดือนเต็ม ไม่ใช่ยอดสะสมทั้งประวัติ**
...
**9. ตัวเลขที่ "เท่ากันไม่ได้" ต้องอธิบาย ไม่ใช่ปล่อยให้เดา**
...
**เทสต์**: `tests/dashboard-metrics.test.js` (57 เคส) คุมทั้งสูตรและ**จุดเชื่อมต่อ** —
มี meta-test เช็คว่า `whRatio` ประกาศก่อนใช้จริง, `completeMonths` ถูกเรียกครบทั้ง 8 จุด,
KPI turnover ไม่ถูกใส่กลับมาเงียบ ๆ, และสูตรกำไรแบบเก่าไม่หลงเหลือในโค้ด
...
**มี Vitest test suite แล้ว** — 888 tests, 25 test files, ทั้งหมด pass
