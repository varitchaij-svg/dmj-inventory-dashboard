...
- **จับคู่ 2 ชั้น**: ตรงเป๊ะหลังถอดเครื่องประดับ → ถ้ายังไม่เจอ ลอง "ชื่อหนึ่งเป็นส่วนหนึ่งของอีกชื่อ"
  (`"KYAW แอ KHALANE"` ↔ `"KYAW แอ KHALANE (คลังสินค้า)"`) · **เข้าข่ายคนเดียวเท่านั้นถึงยอมรับ
  และต้องรายงานใน `who.loose` ก่อนเขียนเสมอ** — จับคู่หลวมเงียบ ๆ = ดาวไปทั้งพันตัวผิดคน
  · เข้าข่ายหลายคน → กำกวม → หยุดทั้งรอบ (ไม่เดา)
...
เทสต์: `tests/product-owner-assign.test.js` (58 เคส — **eval ฟังก์ชันจริงจาก `.gs` ไม่ copy**
