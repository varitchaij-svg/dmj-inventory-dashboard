...
**มี Vitest test suite แล้ว** — 1756 tests, 51 test files, ทั้งหมด pass (+ browser 97/97)
...
- **เครื่องมือตรวจ**: `checkSaleBillInZort("RC-...")` (GAS editor · ชื่อไม่มี `_` ต่อท้ายจึงโผล่ใน
  ...
  · ⚠️ **กดรันจาก dropdown ได้เลยโดยไม่ต้องใส่เลขบิล → ตรวจ "บิลล่าสุด" ให้อัตโนมัติ**
    (`latestSaleBillNumber_` อ่านคอลัมน์ D ~200 แถวท้าย ไม่ getDataRange ทั้งใบ) — จำเป็นเพราะ
    **GAS editor ส่ง argument ไม่ได้** ซึ่งเป็นทางเดียวที่เจ้าของรันตัวนี้จริง ๆ · ไม่รองรับเคสนี้
    = เครื่องมือที่เห็นใน dropdown แต่กดแล้วได้แต่ข้อความบอกให้ใส่เลขที่ใส่ไม่ได้
    (กติกาเดียวกับ `listCategoryBreakdown()`) · ระบุเลขเองยังทำได้เหมือนเดิมทุกประการ
...
เทสต์: `tests/online-sale.test.js` (87 เคส — eval ฟังก์ชันจริงจาก `.jsx` + `.gs` ไม่ copy
...
+ `latestSaleBillNumber_` ที่รันจริงกับชีตจำลอง — คุมว่ากดจาก dropdown โดยไม่ใส่เลขบิลแล้วยัง
ตรวจบิลล่าสุดให้ได้ · อ่านแค่คอลัมน์ D ~200 แถวท้าย · ยังเป็น read-only)
