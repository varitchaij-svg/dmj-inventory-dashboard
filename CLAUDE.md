...
14. **ช่องกรอกตัวเลขที่พรีฟิลด้วยค่าที่ไม่ใช่ 0/ว่าง ต้องมี `onFocus={ev => ev.target.select()}`
    ...
    "รับจริง" ของ `ShipmentRow` (views-analytics.jsx) — พรีฟิลด้วยจำนวนที่ส่งมา ไม่มี auto-select
    ทั้งที่ `fsQty`/`qty` ใน `OrderModal` (views-main.jsx) มี pattern นี้อยู่แล้ว) · เพิ่มช่องกรอก
    ตัวเลขใหม่ที่พรีฟิลค่า **ต้องใส่ `onFocus` นี้เสมอ** ไม่ใช่แค่ตอนเจอบั๊กแล้วค่อยแก้
