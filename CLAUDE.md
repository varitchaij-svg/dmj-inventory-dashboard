...
2. แก้ตาราง **`PRODUCT_OWNER_ASSIGN_PLAN_`** (`appsscript_complete.gs`) ให้ชื่อตรงกับขั้น 1
3. **`previewProductOwnerAssign()`** — อ่านอย่างเดียว บอกว่าจะแจกใครกี่ SKU + จะข้ามอะไรเพราะอะไร
...
เทสต์: `tests/product-owner-assign.test.js` (38 เคส — **eval ฟังก์ชันจริงจาก `.gs` ไม่ copy**
เหมือน auth.test.js) คุมทั้งตรรกะและจุดเชื่อมต่อ (preview ไม่เขียน/ไม่สร้างชีต, ลำดับ lock,
จำนวน call ของ `writeAuditLog_`, เพดานแถว, ค่า default ของ overwrite)
