...
`fetchFromSheet` (app.jsx) ปล่อย mark **`payload:ไบต์แรก`** ตอนได้ไบต์แรก แยก **"รอ GAS คิด"**
(`payload:เริ่ม`→`payload:ไบต์แรก`) ออกจาก **"ดาวน์โหลดก้อน"** (`payload:ไบต์แรก`→`payload:ครบ`)
ใน `BootTrace` · `payload:ครบ` แนบขนาดหลังคลาย gzip (KB) ให้ดูแนวโน้มด้วย
- **ตั้งใจเอามาแค่ตัววัด ไม่แตะ logic อื่น** — ยังไม่เอาแถบ progress-bar UI/จอ checking/timeout
  แยกงานของ 7.6 มา (พวกนั้นคือส่วนที่เสี่ยงตอน rollout เดิม) · error path ใช้ `dmjJson` ตัวเดิมเป๊ะ
  (badjson/ข้อความไทย/`dmj_last_backend_error`) · เบราว์เซอร์ไม่รองรับสตรีม → fallback ไป `dmjJson`
- ⚠️ **เส้นทาง prefetch (`<head>` ของ HTML) ยังไม่วัดไบต์แรก** — มันยิงก่อน `ui.jsx` โหลด จึงไม่ผ่าน
  `dmjJsonProgress` · เคสที่ต้องวินิจฉัยจริง (stampede ตอนเปิดพร้อมกัน) เดินเส้น fetch ปกติซึ่งวัดแล้ว
- **`payload:ครบ (KB)` เป็นไบต์หลังคลายบีบอัด ไม่ใช่ไบต์บนสาย** — ยังตอบไม่ได้ว่า Google gzip
  ให้หรือยัง (ต้องเช็ค `Content-Encoding` จาก endpoint จริง — งานถัดไปของการรีวิวความเร็ว)
- เทสต์: `tests/gasjson.test.js` (eval `dmjJsonProgress` จริงจาก `.jsx` — fallback/นับไบต์สะสม/
  **decode ไทยข้ามขอบ chunk**/error=badjson + meta-test ว่า `fetchFromSheet` ยังเรียกจริง
  และยังปล่อย mark `payload:ไบต์แรก`)
...
✅ **แยก "รอ GAS คิด" ออกจาก "ดาวน์โหลดก้อน" ได้แล้ว (ส.ค. 2026)** — เอา `dmjJsonProgress`
กลับมาเป็นก้อนแรกของ Phase 7.6 (เฉพาะส่วนวัดผล ดูหัวข้อ Phase 7.6) · `fetchFromSheet` ปล่อย mark
**`payload:ไบต์แรก`** แล้ว → `BootTrace` เห็นสองช่วงแยกกัน: `payload:เริ่ม`→`payload:ไบต์แรก`
(รอ GAS คิด: cold start + คิว build) และ `payload:ไบต์แรก`→`payload:ครบ` (ดาวน์โหลด)
· ⚠️ ยังไม่ตอบว่าไบต์บนสายถูก gzip หรือยัง (KB ใน mark เป็นไบต์หลังคลายบีบอัด) — งานถัดไป

### Phase 7.8 — ตาข่ายออฟไลน์ที่ตายมานาน + กัน build เก่าทับ cache (ส.ค. 2026)

**วัดจริงด้วยการรันฟังก์ชันจริง** (docs/REVIEW-ARCHITECTURE-FINAL-2026-08-26.md §16):
`saveToStorage` เดิมเก็บ **ผลของ `enrichData`** ลง localStorage ซึ่งวัดได้ **11.90MB UTF-8
= 22.9MB ตามวิธีนับของ iOS (UTF-16)** เทียบโควตา Safari ~5MB → `setItem` โยน
`QuotaExceededError` **ทุกครั้ง** แล้วถูกกลืนด้วย `console.warn` เฉย ๆ

- ⚠️ **ผลคือตาข่ายออฟไลน์ไม่เคยทำงานเลยบนมือถือ** — `loadFromStorage()` คืน null ทุกครั้ง →
  `data === null` → ติดจอโหลดที่บล็อกทั้งหน้า → **เซิร์ฟเวอร์ช้าเมื่อไหร่กลายเป็น "เข้าไม่ได้"**
  แทนที่จะเป็น "เห็นข้อมูลเก่าแต่ทำงานต่อได้" · เดสก์ท็อป Chrome โควตาใหญ่กว่ามากจึงรอด
  = อธิบายอาการ "เครื่องเจ้าของใช้ได้ แต่มือถือทุกเครื่องเข้าไม่ได้"
- **ตอนนี้เก็บ "ก้อนย่อ" (รูปที่รับมาจาก server ก่อน `enrichData`)** ~2.1MB chars ≈ 4.1MB UTF-16
  · `loadFromStorage` เรียก `enrichData` ให้อยู่แล้ว → **view เห็นผลเท่าเดิมเป๊ะ ไม่มีอะไรหาย**
  · ⚠️ **ต้อง `JSON.stringify(d)` ก่อนเรียก `enrichData` เสมอ** — `enrichData` แก้ `d` **ในที่**
    (กาง `mo`→`monthly` เต็ม 5.8MB + สร้าง `mtoGroups` ที่ซ้ำ object สินค้าทั้งชุด 1.7MB)
  · ⚠️ **ห้ามกลับไปเก็บผลของ `enrichData`** — ทั้งสองก้อนนั้นคำนวณใหม่ได้ฟรีตอนโหลด
- **ทางถอย 2 ชั้น** (`trimForStorage_`): โควตาไม่พอ → ตัด `mo` + ก้อนวิเคราะห์ออกแล้วลองใหม่ →
  ยังไม่พอ → **เก็บของเก่าไว้ ไม่ลบ ไม่ throw** (ของเก่าที่ยังอ่านได้คือสิ่งเดียวที่กันจอบล็อก)
  · โน้ตวินิจฉัยที่ `localStorage.dmj_storage_note_v1`
- **retry ของ payload**: `PAYLOAD_TIMEOUT_RETRY_MS` (30s) **ต้องมากกว่า `_BUILD_LOCK_WAIT_MS`
  (25s) เสมอ** — ของเดิม 20s **สั้นกว่าคิว build ของ server** → คำขอที่กำลังต่อคิวถูกตัดทิ้ง
  ก่อนคิวปล่อยของเสมอ = ลองใหม่กี่ครั้งก็ไม่มีวันสำเร็จ และ **GAS ไม่ยกเลิก execution ตาม
  client abort** → คำขอที่ถูกตัดยังกินสล็อตจนจบ = ยิ่งลองยิ่งถ่วง · ลดจำนวนครั้ง 4 → 3

**กัน build เก่าทับ cache ชั้นสด (ความถูกต้อง ไม่ใช่ความเร็ว)**: build กิน ~10 วิ ระหว่างนั้น
มีคนบันทึกได้ (doPost จับ `getScriptLock` คนละตัวกับ build lock) → `invalidateCache_` ล้าง cache
แล้ว build ที่อ่านชีตไปก่อนหน้าเขียนทับ → **cache ชั้นสด = ข้อมูลก่อนบันทึก** + เส้น HIT ปั๊ม
`lastModified` สดทับอีก → `shouldRejectConflict_` ปล่อยผ่าน → **เขียนทับงานคนอื่นเงียบ ๆ**
ได้นานถึง TTL 180 วิ
- CacheService **ไม่มี compare-and-set** → กันด้วย **เทียบ ts ก่อนเขียน** (ไม่เพิ่มล็อกใหม่):
  `_tsBefore` ก่อน build · หลัง build ถ้า ts ขยับ (`>`) = `_staleBuild` → **ไม่เขียนชั้นสด**
  · ⚠️ ใช้ `>` ไม่ใช่ `!==` — `getSheetLastModified_` ถอยไป `DriveApp` ได้เมื่อ property หาย
    ซึ่งกระเพื่อมสองทาง · `!==` จะทำให้ **ไม่เขียน cache เลยตลอดกาล** = build ทุกคำขอ
- **ชั้นสำรองยังเขียนเสมอ** (ไม่เคยถูกปั๊ม ts สด จึงไม่ทำให้ conflict หลุด) — ไม่งั้นกลับไป stampede
- `data.lastModified` ตั้งเป็น `_tsBefore` เสมอ (ตอนเริ่มอ่านชีต ไม่ใช่ตอนประกอบเสร็จ)
- **`keepWarm_` มี race เดียวกันเป๊ะ** → ใส่ guard ตัวเดียวกัน
- เทสต์: `tests/startup-resilience.test.js` (21 เคส — eval จริงจาก `.gs`/`.jsx` ไม่ copy)
  มีเคสที่ **พิสูจน์ว่า race มีจริง** โดยรันลำดับเดิมแบบไม่มี guard แล้วเห็นข้อมูลก่อนบันทึก
  ไปนั่งใน cache · + เคส chunk หลุดตัวเดียว = ทั้งก้อน MISS (ข้อจำกัดเชิงโครงสร้างที่ยังอยู่)

### Phase 8 — เลิกให้ "เปิดแอป" ผูกกับ `buildFullData_` (ส.ค. 2026)

**หลักฐานที่ตั้งต้น**: `buildFullData_` วัดจริง **7.6 วิ** (container อุ่น — `perfMeasureBuild`)
ถึง **18.0 วิ** (เย็น — วัดข้างใน `keepWarm_` ซึ่งใช้ทั้งรอบ 26.65 วิ) · และ **ContentService
บัฟเฟอร์คำตอบ** → ไบต์แรกถึง browser ต่อเมื่อ `doGet` คืนค่าแล้ว ⇒ **ทุก cache miss ผู้ใช้
คนนั้นจ่ายเวลานั้นเองเต็ม ๆ** (trace เครื่องผู้ใช้: รอ payload 19.3 วิ / ดาวน์โหลดจริง 607 ms)
· `keepWarm_` ติดตั้งและยิงอยู่จริง — **จึงพิสูจน์แล้วว่า keep-warm ไม่ใช่กลไกรับประกัน**

#### Phase A — ล้าง cache ให้ตรงกับสิ่งที่เปลี่ยนจริง

- **`PAYLOAD_SOURCE_SHEETS_`** = ตารางชี้ขาดว่า payload ประกอบจากชีตไหนบ้าง (ไล่จาก read
  function ทุกตัวที่ `buildFullData_` เรียก) · การเขียนที่ไม่แตะชุดนี้เลย **ห้ามล้าง payload cache
  และห้าม bump `dmj_last_write_ts`** — bump = ทุกเครื่องที่ถาม `action=ver` เห็นว่า "เปลี่ยนแล้ว"
  แล้วโหลดก้อนเต็มซ้ำทั้งที่ข้อมูลเหมือนเดิมทุกไบต์
- ย้าย dispatch ใบเสนอราคา/ใบแจ้งหนี้/ใบกำกับ **9 ตัว** ขึ้นเหนือ `invalidateCache_(true)`
  (ที่เดียวกับ `markNotiRead`/`setProductOwner`) + ถอด `invalidateCache_()` ที่ซ้ำซ้อนออกจาก
  ตัวจัดการใบเสนอราคาและทะเบียนสินค้า · รวมการลบคีย์ที่เขียนซ้ำ 5 จุดมาที่ **`invalidateQuoteCaches_()`**
- ⚠️ **`saveThresholds_` ยังต้องล้างเหมือนเดิม** — `thresholds` อยู่ใน payload แต่มาจาก
  **Script Property ไม่ใช่ชีต** (ตารางที่ดูแต่ชีตจะพลาดข้อนี้) · **`createSaleBill` ห้ามย้าย**
  — มันเรียก `deductFrontStoreForSale_` ที่เขียนคอลัมน์ G ของชีตสต็อกจริง
- ⚠️ **`invalidateCache_` ทั้งก้อนอยู่ใน try เดียวที่จบด้วย `catch { ignore }`** — อะไรโยนข้างใน
  ก็ข้ามการล้าง payload ทั้งหมด**แบบเงียบ ๆ** = cache ค้างของก่อนบันทึก ซึ่งคือคลาสบั๊ก
  "เขียนทับงานคนอื่นเงียบ ๆ" · บล็อกที่เพิ่มใหม่ต้องมี try/catch **ของตัวเอง** เสมอ
- ⚠️ **"stock write ล้างเฉพาะ stock overlay" ยังทำไม่ได้ และไม่ได้ทำ** — `data.products[]`
  ในก้อนเต็ม**มีจำนวนสต็อกอยู่ข้างใน** · ไม่ล้าง = เสิร์ฟจำนวนเก่าได้ถึง 180 วิ **และ**
  เส้น HIT ปั๊ม `lastModified` สดทับ → `shouldRejectConflict_` ปล่อยผ่าน → ทับงานคนอื่นเงียบ ๆ
  · จะทำได้ต่อเมื่อมีก้อน catalog ที่**ไม่มีจำนวนสต็อก** = ต้องทำ Phase B ก่อน (ลำดับคือ B→A)

#### Phase B — `action=boot`: ก้อนเปิดแอปที่ไม่แตะ `buildFullData_`

- **`buildBootData_()`** อ่านเฉพาะ **`BOOT_SOURCE_SHEETS_`** (8 ชีต — เป็น **subset ของ
  `PAYLOAD_SOURCE_SHEETS_` เสมอ** มีเทสต์บังคับ) · ตัดทิ้ง: ยอดขายรายเดือน/รายวัน
  (`products[].mo` = ก้อนใหญ่สุดและ**โตขึ้นเองทุกเดือนไม่มีเพดาน**), ประวัติซื้อ, ประวัติโอน, MTO
- cache 2 ชั้นของตัวเอง (`dmj_boot_*` 180 วิ · `dmj_bootstale_*` 30 นาที) · **ไม่แยกตาม role**
  (คีย์ที่ `PAYLOAD_VARIANT_DROPS_` ตัดตาม role ไม่มีอยู่ในนี้ตั้งแต่แรก) = build ครั้งเดียวอุ่นทั้งร้าน
  · ⚠️ เพิ่มคีย์ที่ role เห็นไม่เท่ากันเข้ามาเมื่อไหร่ **ต้องกลับไปแยก cache ตาม variant ก่อน**
- ⚠️ **ห้ามเรียก `buildFullData_` จากเส้นทาง boot เด็ดขาด** ทั้ง HIT และ MISS
- ⚠️ **ใช้ `acquireBuildLock_(0)` เท่านั้น ห้ามต่อคิว `_BUILD_LOCK_WAIT_MS` (25 วิ)** —
  จุดประสงค์ทั้งหมดคือไม่รอ build ก้อนใหญ่ ถ้ามารอคิวเดียวกันก็เท่ากับไม่ได้แก้อะไร
- ⚠️ **ก้อน boot ไม่ใส่คีย์ `soldQty`/`soldRev`/`mo`/`monthly` เลย (ไม่ใช่ใส่ 0)** — 0 คือตัวเลข
  ผิดที่หน้าตาเหมือนข้อเท็จจริง · ติดธง **`_boot`** ให้ฝั่ง client รู้ว่ายังคิดเลขจากยอดขายไม่ได้
- `keepWarm_` อุ่นก้อน boot **ก่อน** ก้อนใหญ่ (ไม่แตะ trigger ใด ๆ) · `invalidateCache_`
  ล้าง**ชั้นสด**ของ boot ด้วย แต่ **ห้ามแตะชั้นสำรอง** (ไม่งั้นกลับไป stampede)
- **ฝั่ง client**: `fetchBoot()` ยิง **คู่ขนาน** กับก้อนเต็ม (ไม่ใช่แทนที่) → **`mergeBootData`
  รวม ไม่ใช่ทับ**: มี snapshot เดิม → ทับเฉพาะของที่เปลี่ยนบ่อย ยอดขายคงเดิม (**ไม่มีอะไรด้อยลง**)
  · ไม่มี snapshot → ใช้ boot เป็นฐาน + ธง `_boot`
  · ⚠️ **`fetchBoot` ห้าม `saveToStorage`** — ทับ snapshot เต็มด้วยก้อนที่ไม่มียอดขาย =
    เปิดแอปรอบหน้าข้อมูลด้อยลงถาวร
  · ⚠️ **`fullAppliedRef` กันก้อน boot ที่มาช้าเขียนทับก้อนเต็ม** (เช็ค 2 ชั้น รวมข้างใน `setData`
    เพราะระหว่าง `then` กับ `setData` ก้อนเต็มมาถึงได้) · `fetchFromSheet` ตั้งธง **ก่อน** `setData`
  · ⚠️ **ต้องเช็ค `d._boot` ก่อนใช้เสมอ** — GAS รุ่นเก่าไม่รู้จัก `action=boot` แล้วคืนก้อนเต็มมาแทน
- เทสต์: `tests/cache-domains.test.js` (55) + `tests/boot-payload.test.js` (36) — eval ของจริง
  จาก `.gs`/`.jsx` ไม่ copy · browser **`เปิดแอปด้วยก้อน boot (payload เต็มค้าง)`** ใช้โหมด
  `bootonly=1` ที่ทำให้ payload เต็ม**ค้างไม่ตอบเลย** — เป็นเคสเดียวที่แยก "ไม่ผูกกับ
  buildFullData_ จริง" ออกจาก "บังเอิญเร็ว" · ⚠️ harness ต้องมี branch `action=boot` ไม่งั้น
  เส้นทางนี้ไม่เคยถูกทดสอบเลยทั้งที่เทสต์ยังเขียว

**ยังไม่ทำ (ตั้งใจไม่บันเดิลมาด้วย)**: ย้ายก้อนหนักไป endpoint ต่อ view (Phase C) และ
ถอด `buildFullData_` ออกจากเส้นทางเปิดแอปจริง (Phase D — ต้องมี telemetry จาก production
ยืนยันก่อนว่าไม่มีใครเดินเส้นเก่าแล้ว) · **เหตุผลที่ไม่รวบ**: Phase 7.6 เคยขึ้น 3 ก้อนพร้อมกัน
แล้วร้านเข้าไม่ได้ทั้งร้านโดยแยกไม่ออกว่าอะไรพัง
...
- ⚠️ **ตัวเติมรูปห้ามแตะ `price`/`qty`/`name`** — ร่างเก็บค่าที่ผู้ใช้ตั้ง/แก้เอง ทับเมื่อไหร่ =
  งานที่ทำไว้หายเงียบ ๆ · เส้นทาง `editQuote` ก็ยึดหลักเดียวกัน (ห้ามทับราคา) — หารูป/หมวด
  จาก catalog ได้ แต่ **ราคาต้องใช้ค่าที่โหลดมา** ไม่รวมเป็นฟังก์ชันเดียวกับตัวเติมรูป
...
#### แก้ไขใบเสนอราคาแล้ว "ยอดต้องเท่าใบเดิม" (ส.ค. 2026)

เจ้าของแจ้ง (+ ภาพหน้าจอ): กด "แก้ไข" ใบเสนอราคาแล้วยอดรวมในฟอร์ม**ไม่เท่าใบเดิม** · ต้นเหตุ:
โหมดแก้ไข**เดิม** "คืนราคาตั้งจาก catalog ปัจจุบัน" แล้วให้ฟอร์ม/backend หักส่วนลดขายส่งใหม่ →
ยอดเพี้ยนได้ 2 ทางพร้อมกัน: (1) ราคา catalog เปลี่ยนไปจากตอนออกใบเดิม (2) หักส่วนลดซ้ำ ·
และราคาสุทธิที่ ZORT เก็บ (`pricepernumber`) **หาราคาปลีกเดิมกลับไม่ได้** (factor พับ retail กับ
ส่วนลดเข้าด้วยกัน — invert แล้วตัดกันหมด)

- **วิธีแก้ = โหลด "ราคาสุทธิ" จาก ZORT ตรงๆ (`it.price`) + ธง `pricesFinal` กันหักส่วนลดซ้ำ**
  → ยอดรวม = ผลรวมราคาสุทธิ = ยอดใบเดิมเป๊ะ (คลาดได้แค่ระดับสตางค์จากการปัดต่อบรรทัด)
- **`pricesFinal` เดินครบทั้งเส้น** (หลุดจุดใดจุดหนึ่ง = ยอดเพี้ยนกลับ เงียบ ๆ):
  cart init ใช้ `it.price` · `computeBillTotals(cart, { …, pricesFinal: !!editQuote })` ·
  `buildQuotePayload` ส่ง `pricesFinal` · backend `editQuotation` ส่งต่อให้ `computeBillTotalsGs_`
  · ทั้ง `computeBillTotals` (+ สำเนา `helpers.js`) และ `computeBillTotalsGs_` เช็ค
  `!opts.pricesFinal && … >= 6` เพื่อข้ามส่วนลดขายส่ง/ขั้นบาท
- ⚠️ **`createQuotation` ห้ามส่ง `pricesFinal`** — ใบใหม่รับราคาปลีก ต้องหักส่วนลดจริง (มีเทสต์กันไว้)
- ราคาต่อชิ้นในฟอร์มแก้ไขจึงเป็น**ราคาสุทธิ** (ไม่ใช่ราคาปลีก) — แบนเนอร์เตือนอธิบายไว้แล้ว
  ถ้าจะปรับราคาให้พิมพ์เป็น**ราคาสุทธิ**ที่ต้องการ (แก้จำนวนก็คูณราคาสุทธินั้นตรงๆ ไม่หักซ้ำ)
- เทสต์: `tests/quote-edit-amount.test.js` (12 เคส — behavioral `computeBillTotals` + meta-test
  คุมจุดเชื่อมต่อทั้งเส้น รวมกัน `createQuotation` เผลอส่ง `pricesFinal`)
...
7. **กดส่งทีละใบ (`finalizeShip` → `transferStock`) ยังไม่มีตัวกันโอนซ้ำฝั่ง GAS** (ไม่มี tid) —
...
### กดส่งครั้งเดียวแต่ได้ TF ซ้ำ + สต็อกหักซ้ำ — ดับเบิลแท็บ ConfirmModal (แก้แล้ว ส.ค. 2026)

อาการ: กดส่งของทีละใบ **ครั้งเดียว** แต่ขึ้นรายการโอนซ้ำ 2 ใบต่อ SKU (เจ้าของเห็นในแท็บติดตาม
`TF-...-066`/`067` เป็น OE003 qty 6 ทั้งคู่ · `TF-...-064`/`065` เป็น OE002 **qty 1 แล้ว qty 0**)
- **ลายเซ็นชี้ขาด**: qty `1` แล้ว `0` ของ order เดียวกัน = ถูกส่ง **2 ครั้งซ้อน** — ครั้งแรก
  `transferStock` หักหน่วยสุดท้าย (คลัง→0) ครั้งที่ 2 เจอคลัง 0 → `actual=min(qty,0)=0` โอน 0 ชิ้น
  · แต่ละครั้งเขียน TF ใหม่ (refNum จาก `getLastRow`) → เลขวิ่งต่อกัน
- **ต้นเหตุ**: ปุ่ม ✅ ใน **`ConfirmModal` (views-main.jsx)** ไม่มีตัวกันดับเบิลแท็บ → มือถือแตะ
  ซ้ำเร็ว ๆ (ghost-tap) ยิง `onConfirm` 2 ครั้งใน tick เดียว → `doShip` อ่าน `order` จาก closure
  เดิมทั้งคู่ (`setShipConfirm(null)` เป็น **state = async** ไม่ทันตัดครั้งที่ 2) → `finalizeShip`×2
  → `transferStock`×2 · **`transferStock` ทีละใบไม่มี tid กันซ้ำฝั่ง GAS** (ข้อ 7 ข้างบน) จึงลงซ้ำจริง
- **แก้ที่ต้นเหตุคือ ConfirmModal** (ใช้ทุก confirm flow — ส่ง/ลบ/ส่งทั้งหมด) ด้วย
  **`React.useRef` ไม่ใช่ state** — ref อัปเดต **synchronous** การกดครั้งที่ 2 ใน tick เดียวจึงเห็น
  `true` แล้วถูกตัด (state อัปเดต async จะยังเห็น `false` → กันไม่ได้จริง) · ปุ่มเรียก
  `handleConfirm` (ตัวกัน) **ห้ามเรียก `onConfirm` ตรง ๆ** · รีเซ็ต ref ทุกครั้งที่ `open` เป็น true
- **ชั้นที่ 2 (เผื่อ caller อื่น)**: `finalizeShip` มี `shipInflightRef` (Set ของ order.id) ตัดทันที
  ถ้ากำลังโอน order เดียวกันอยู่ · **ปล่อยล็อกครบทุกทางออก** (unreadable/คลังไม่พอ/สำเร็จ = delete 3 ที่)
  ไม่งั้นกดส่งซ้ำไม่ได้ตลอดไป
- ⚠️ **ยังไม่แก้ให้ `transferStock` มี tid ฝั่ง GAS** (ข้อ 7) — ตอนนี้ปิดช่องดับเบิลแท็บที่ต้นทาง
  แล้ว ซึ่งเป็น trigger เดียวจริง ๆ (`syncStockDeduct` ไม่ auto-retry) · ถ้าจะกันระดับ backend
  ให้ทำแบบเดียวกับ `cid`/`tid` (งานต่อ)
- เทสต์: `tests/transfer-idempotent.test.js` (+4 เคส — meta: ConfirmModal กันด้วย ref ก่อนเรียก
  onConfirm, ปุ่มเรียก handleConfirm, รีเซ็ตตอนเปิด, finalizeShip ปล่อยล็อกครบ 3 ทางออก)
...
- เทสต์: `tests/stockcount-supplier-save.test.js` (meta-test สแกนต้นทางจริง — ปุ่มซัพพลายเออร์
  ...
  `handleSaveSupplier` ถูกลบ, `syncLockData` อ่าน `dmjJson`) · **+ browser test เดินจริง**
  (`นับ stock คลัง auto-save (warehouse)` — เข้าโหมดตามซัพ→เลือก ACME→กด +5→รอ 3 วิ→การ์ดขึ้น
  "บันทึกแล้ว") พิสูจน์ว่า auto-save คอมมิตเข้าคลังจริงบนเบราว์เซอร์

### การ์ดนับสต็อกต้องบอกชัดว่า "บันทึกเข้าคลังแล้ว" ไม่ใช่แค่ ตรง/ไม่ตรง (แก้แล้ว ส.ค. 2026)

เจ้าของถาม (พร้อมภาพ): "มันบันทึกแค่ที่นับตรงแต่แก้จำนวนจริงไม่ได้หรอ · และยังไม่เห็น autosave"
· ต้นเหตุคือ **UX สื่อสารผิด** ไม่ใช่ตรรกะเซฟ (confirmStockCount เซ็ตค่าคลังแบบ absolute อยู่แล้ว
ทั้ง match/mismatch → นับต่างจากระบบ = แก้คลังจริงเสมอ):
- การ์ดเดิมโชว์ badge **"!" แดง** ตอนนับต่างจากระบบ (mismatch) → ผู้ใช้อ่านว่า "error/เซฟไม่ได้"
  ทั้งที่ mismatch คือหน้าที่ของการนับสต็อก (แก้คลังให้ตรงของจริง)
- ไม่มีป้าย "บันทึกแล้ว" ถาวรต่อการ์ด · แถบ "✓ บันทึกแล้ว" ข้างปุ่มโชว์แค่ 3 วิแล้วหาย ·
  ปุ่มยังขึ้น "บันทึก (3)" ตลอดแม้เซฟครบ → เข้าใจผิดว่ายังไม่ได้เซฟ
- **แก้**: เพิ่ม state **`savedQtys`** (sku → ค่าที่เซฟเข้าคลังไปแล้วจริง) อัปเดตในทุก handler ที่
  เซฟสำเร็จ · การ์ดคิด `saved = has && savedQtys[sku] === num` → badge เขียว ✓ (saved) / ส้ม ⏳
  (รอเซฟ) **ไม่มี ! แดงอีก** · count-result: saved → "✅ บันทึกแล้ว · แก้คลังเป็น N (เดิม M)" ·
  ปุ่มโชว์ `suppUnsavedCount` (ที่ยังไม่เซฟ) → เซฟครบขึ้น "✓ บันทึกครบแล้ว"
- ⚠️ **`savedQtys` เก็บ "ค่า" ไม่ใช่แค่ "เคยเซฟไหม"** — แก้เลขใหม่หลังเซฟ = ไม่ตรง → กลับเป็น
  "รอบันทึก" จนกว่า auto-save รอบใหม่จะยิง (ต่างจาก `savedSkus` ที่เป็น Set เฉย ๆ)
- ⚠️ **`sys` (คลังจากpayload) ยังเป็นค่าเดิมจนกว่าจะ refetch** — หลังเซฟ 5 (เดิม 25) การ์ดจึงเขียน
  "แก้คลังเป็น 5 (เดิม 25)" จาก savedQtys ไม่ใช่จาก sys (ซึ่งยังค้าง 25) เพื่อไม่ให้โชว์ mismatch หลอน
...
### กรองต่อด้วยสีจากแท็บ 🔍/🏷️ + ค้นชื่อสั้น ๆ ไม่ควรกวาดคำที่สะกดปนกัน (ส.ค. 2026)

เจ้าของลองใช้งานจริงหลัง merge Phase 1-4 แล้วแจ้ง 2 เรื่อง: (1) อยากกดเลือกสีต่อจากชื่อ/หมวดได้
เป็นการกรองอีกขั้น ไม่ใช่แค่แท็บ 🎨 เดี่ยว ๆ (2) สินค้าชื่อเฉพาะสั้น ๆ อย่าง "สน" พิมพ์ค้นแล้วขึ้น
ทุกตัวที่สะกดปนคำนี้ (เช่น "สนิม") เพราะภาษาไทยไม่มีช่องว่างคั่นคำ ทำให้ substring กว้างเกินไป

**1. กรองต่อด้วยสี** — แถบ **"🎨 กรองต่อด้วยสี"** โผล่เฉพาะแท็บ 🔍/🏷️ (ไม่ใช่ 🏭/🎨 — 🎨 เดี่ยว ๆ
ยังทำงานแบบเดิมทุกประการ ไม่ถูกแตะเลย) ใช้ **`checkColorFilter`** (สีเดียว หรือ null) เทียบกับ
**`checkSkuColorMap`** (sku→ชื่อสี จาก `checkBase`) ก่อนเพิ่มเข้า `checkPicked` — **ไม่ใช่แหล่ง SKU
แหล่งใหม่** แค่ narrow ผลของแท็บที่เปิดอยู่ตอนนั้น (ทำตาม pattern เดียวกับหน้าหลักที่มีอยู่แล้ว:
`refineBase`→`colorChips` ให้หมวดที่เลือกไว้จำกัดตัวเลือกสีที่ตามมา)
- **`checkKeywordEffSkus(r)` / `checkCategoryEffSkus(c)`** = จุดคำนวณ "ผลจริงหลังกรอง" เดียวของ
  ทั้งระบบ — ทั้งตอน render ปุ่ม/นับจำนวน และตอนคำนวณ `checkSourceLabel` เรียกตัวเดียวกัน
  ⚠️ **`checkSourceLabel` ต้องใช้ 2 ฟังก์ชันนี้ ห้ามเช็ค `c.skus`/`r.skus` ดิบ** — เพิ่มแค่
  "หมวด ∩ สี" (ไม่ใช่ทั้งหมวด) แล้วเช็คแบบเดิมจะไม่มีวัน "ครบทุกตัวใน checkPicked" เพราะเราตั้งใจ
  เพิ่มแค่ส่วนที่ตรงสี ไม่ใช่ทั้งกลุ่ม → sourceLabel จะไม่รายงานว่ามีส่วนร่วมเลยทั้งที่เพิ่งเลือกไป
  จริง (คลาสบั๊ก "หายเงียบ" เดียวกับที่ทั้งไฟล์นี้เตือนไว้ตลอด) — segment ที่ผ่านตัวกรองสีจะเติม
  `" (สีX)"` ต่อท้ายด้วยเพื่อบอกว่าใช้ตัวกรองอะไรตอนกด
- resetCheckPicker ล้าง `checkColorFilter`/`checkKeywordExpand` ด้วย (กันค้างข้ามการเปิดโมดัลใหม่)

**2. ชื่อ/รหัสตรงคำค้นเป๊ะมาก่อน substring เสมอ** — `checkKeywordResult` เพิ่ม tier 0: ถ้ามีสินค้า
ที่ **ชื่อหรือ SKU ตรงกับคำค้นทั้งคำเป๊ะ** (ตัดช่องว่าง/ตัวพิมพ์ใหญ่-เล็ก) ให้ถือว่านั่นคือสิ่งที่
ต้องการเป็นค่าเริ่มต้น (แม่นกว่า substring ที่กวาดคำอื่นที่บังเอิญสะกดปนมาด้วย)
- ⚠️ **ห้ามซ่อนตัวที่สะกดคล้ายกันถาวร** — เก็บไว้ใน `r.broaderSkus` (exactSkus ⊆ strictSkus เสมอ)
  พร้อมปุ่ม **"+ มีสินค้าอื่นสะกดคล้ายกันอีก N รายการ (แตะเพื่อรวม)"** ให้กดรวมเองได้เสมอ
  (`checkKeywordExpand`, เก็บด้วย **term string** ไม่ใช่ index เพราะ index ขยับทุกครั้งที่แก้กล่อง
  ข้อความ) — กันเคส "หาไม่เจอทั้งที่รู้ว่ามี" ซึ่งแย่กว่าเห็นเกินจากเดิม
- ใช้ tier นี้เฉพาะตอนที่ `exactSkus.length < strictSkus.length` (มี noise จริงให้ตัด) — ถ้าตรงเป๊ะ
  เป็นตัวเดียวที่ match อยู่แล้ว ไม่ต้องมี `exact`/`broaderSkus` เพิ่ม พฤติกรรมเดิมเป๊ะ
- ข้อความ "ไม่พบสินค้า" แยก 2 กรณี: **ไม่พบเลยทั้งเทอม** (เดิม) กับ **พบแต่ไม่ตรงสีที่กรองไว้**
  (ใหม่ — "⚠️ พบสินค้าแต่ไม่มีสีX") ไม่ให้ปนกันจนเข้าใจผิดว่าไม่มีสินค้านั้นเลย
- เทสต์: เพิ่มใน `tests/stockcheck-keyword.test.js` (behavioral ของ tier 0 + `checkKeywordEffSkus`/
  `checkCategoryEffSkus` + `checkSourceLabel` กรณีกรองสี + meta-test สายส่ง UI) · browser test ใหม่
  ใน `tests/browser/run.cjs` (fixture `PIN010`="สน"/`PIN011`="ต้นสนประดิษฐ์"/`PIN012`="สนใบเงิน"
  หมวดใหม่ "ต้นไม้ประดิษฐ์" — ตั้งใจแยกจาก "ดอกไม้"/"แดง" ที่เทสต์ก่อนหน้าใช้อยู่แล้ว กันชนกัน)
  ยืนยันบนเบราว์เซอร์จริงว่าค้น "สน" ได้แค่ PIN010 + กด "รวม" ได้ครบ 3 + หมวด∩สีเงินเหลือ PIN012
  ตัวเดียว · ⚠️ ชื่อหมวด/สีใหม่ชนกับ sidebar "หมวดหมู่"/แถบกรองสีของหน้าหลักใน harness (ไม่มี CSS
  ซ่อนบนจอแคบเหมือน production) เหมือนเคส "ดอกไม้" ก่อนหน้า — ใช้ `.last()` เพื่อเลือก element
  ในโมดัล (render ทีหลัง sidebar เสมอ) ไม่ใช่ `.first()`
...
- **มือถือ/iPad**: `useIsMobile(700)` สลับตาราง 9 คอลัมน์ → การ์ดต่อวัน · ไทล์ตัวเลขใช้ `statCols`
  ที่ตั้งเป็นขั้น (มือถือ 2 · iPad/จอแคบ `useIsMobile(1150)` 3 · จอกว้าง 6) **ไม่ใช่ auto-fit**
  — auto-fit ที่ความกว้าง iPad ให้ 5 ช่อง + ตกไปอีกบรรทัดเดี่ยว ๆ ดูเหมือนไทล์หลุดมา 1 อัน
  · ตารางบนจอกว้างเลื่อนแนวนอน**ในกล่องตัวเอง** (`overflowX:auto`) ไม่ดันทั้งหน้า
...
- ⚠️ **วันก่อนวันเข้างานจริง (`createdAt`) ห้ามนับเป็น "ขาด"/"มีกะ"** (แก้ ส.ค. 2026 —
  พบตอน scrutinize) — เดิม `absent`/`daysScheduled` ไล่ทุกวันในเดือนแบบไม่มีพื้นวันเข้างานกันไว้
  → คนเข้าใหม่กลางเดือนโดนนับ "ขาด" ทุกวันก่อนหน้าที่ยังไม่เป็นพนักงานเลย ซึ่งเป็นตัวเลขที่
  เจ้าของเอาไปคุยเรื่องเงินเดือนพอดี · ตอนนี้ `attendanceMonthlySummaryBuild_` คำนวณ
  `hireFloor = attDateKey_(new Date(st.createdAt))` แล้ว gate ทั้ง `shift` (เป็น `null` ก่อนวันนั้น)
  และ `absent` (พึ่ง `shift` อยู่แล้ว ไม่ต้องเช็คซ้ำ) · ส่ง `beforeHire` ไปกับทุกวันให้ frontend
  ด้วย (`attDayNote` ขึ้น "ยังไม่เข้าทำงาน" แยกจาก "วันหยุด") · `createdAt` ว่าง/parse ไม่ออก →
  ไม่จำกัดวัน (ปลอดภัยกว่าเดาวันเข้างานผิด) · **ยังไม่รองรับวันลาออกกลางเดือน** — ชีต "พนักงาน"
  ไม่มีคอลัมน์วันที่ออก มีแต่ `status` ซึ่งกรองคนที่ไม่ active ออกจากทั้งรายงานอยู่แล้ว
- ⚠️ **cache 5 นาทีต่อเดือน** (`dmj_attmonthly_<month>`, เหมือน `staffPerfHandler_`) — เดิมไม่มี
  cache เลย ทุกกดลูกศรเปลี่ยนเดือน/รีเฟรชอ่านชีตลงเวลาทั้งใบ (`getRange` กว้าง 17 คอลัมน์) แล้ว
  รัน `attSummarize_` ต่อคน×วันใหม่หมด · แยก `attendanceMonthlySummaryBuild_` (คำนวณล้วน คืน
  object) ออกจาก `attendanceMonthlySummaryHandler_` (session check + cache) เพื่อ cache เป็น
  JSON string ตรง ๆ · **กด 🔄 ส่ง `fresh:true` ข้าม cache เสมอ** (`load(month, true)`) ·
  **`fixAttendanceHandler_` ต้องล้าง `dmj_attmonthly_<เดือนของวันที่แก้>` ทุกครั้ง** ไม่งั้น
  เจ้าของแก้เวลาย้อนหลังแล้วเปิดรายงานไปดูยังเห็นเลขเก่าค้างจนกว่า cache จะหมดอายุเอง
- ⚠️ **เส้น/ป้ายเวลากะในโหมด "ทั้งหมด" เป็นค่าที่พบบ่อยสุดข้ามทุกคน ไม่ใช่กะจริงของทุกคน**
  (`t.shiftStart`/`t.shiftEnd` มาจาก majority vote ใน `attReportAgg`) — หน้าร้าน/คลังกะเริ่ม
  ไม่เท่ากันได้ · sub-label ไทล์และ legend กราฟเติมคำว่า "(ที่พบบ่อยสุด)" เมื่อ `!one` (ไม่ได้
  เลือกคนเดียว) กันเข้าใจผิดว่าเป็นกะจริงของทุกคนในกลุ่ม
- `ROLE_TH_ATT` (views-attendance.jsx) ต้องมีครบทุก role ใน `STAFF_ROLE_TH_` ฝั่ง `.gs`
  (รวม `dev`/`storedevice`) ไม่งั้น role พวกนั้นขึ้นเป็นค่าดิบในตารางรายคน
- เทสต์: `tests/attendance-report.test.js` (eval ฟังก์ชันจริงจาก `.jsx`/`.gs` ไม่ copy — คุม
  hire-floor/cache/invalidation/refLabel เพิ่มแล้ว) + browser test 2 เคส (จอ 390px และ 1024px
  — เลือกคนแล้วต้องได้รายวันของคนนั้นจริง, ไม่เหลือข้อมูลคนอื่น, และ**หน้าไม่ล้นแนวนอน**) ·
  fixture ของ browser test **ต้องมี `days[]` จริง** ไม่งั้นค่าเฉลี่ยทุกช่องเป็น "—" แล้วเทสต์
  เขียวโดยไม่ได้ทดสอบส่วนที่ทำใหม่เลย

## 🔒 ผลตรวจระบบ 5 ก.ย. 2026 — ชุดที่ 1: สิทธิ์/ตัวตน + ความน่าเชื่อถือของสถานะ

รายงานตรวจระบบ (จัดทำโดย Johny, ตรวจจาก master `03027b7`) พบ 14 ประเด็น · **P1 = 7 ข้อ
ยืนยันจากโค้ดจริงแล้วทั้งหมด ไม่มี false positive** · เจ้าของเลือกทำก้อนแรก = **F02 + F04 + F06**
· ตั้งใจ **ไม่ยกทั้งชุด 1 (F01–F04) มาพร้อมกัน** เพราะ F01 เป็นเส้นทางล็อกอินซึ่งคือจุดเดียวกับที่
Phase 7.6 เคยขึ้น 3 ก้อนพร้อมกันแล้วร้านเข้าไม่ได้ทั้งร้าน แยกไม่ออกว่าอะไรพัง

### F02 — กดระงับพนักงานแล้วเขายังทำงานได้ต่ออีก 30 วัน (แก้แล้ว)

`resolveSession_` ตรวจแค่ **ตัว session เอง** (หมดอายุ / ถูก revoke) **ไม่เคยดู `status` ของ
แถวพนักงาน** แล้วคืน staff object ตามปกติ · `saveStaffHandler_` ตอนเจ้าของกดระงับ ล้างแค่
session **cache** ไม่ได้ revoke token · และ `canDoOrNull_` เขียนคอมเมนต์ไว้ว่า *"ไม่ต้องเช็ค
status ซ้ำ เพราะ resolver ตรวจแล้ว"* ซึ่ง**เป็นสมมติฐานที่ผิด** → คนที่ถูกระงับยังสั่งของ/โอนสต็อก/
ออกบิลได้จนกว่า session หมดอายุเอง (30 วัน) ทั้งที่หน้าจอเจ้าของขึ้นว่าระงับไปแล้ว

- **`sessionInactiveOrNull_(sess)`** = ด่านเดียวของทั้งระบบ · ใช้ 2 ที่: `canDoOrNull_`
  (ด่านกลางของ doPost) และ **`handleOrder_`** ซึ่งเป็น **doGet จึงไม่ผ่านด่านกลาง** แต่เขียนชีตจริง
  · ⚠️ **เขียนเงื่อนไขซ้ำแทนการเรียก helper เมื่อไหร่ = แก้ที่หนึ่งอีกที่ยังรั่วโดยไม่มีอะไรเตือน**
- ⚠️ **ไม่ผูกกับ `REQUIRE_LOGIN` โดยเจตนา** — "กดระงับแล้วต้องมีผลทันที" คือความถูกต้องพื้นฐาน
  ไม่ใช่ฟีเจอร์ที่รอ rollout · ครอบเงื่อนไขนี้ด้วย flag เมื่อไหร่ = ช่องโหว่กลับมาทันที
- ⚠️ **ต้องอยู่เหนือตาราง `IMMEDIATE_GATE_*` และ `ROLE_ACTIONS_`** — role owner ที่ถูกระงับ
  ต้องไม่ข้ามด่านนี้ด้วย `isAdminRole_`
- ⚠️ **`status` ว่าง = "ไม่รู้" ไม่ใช่ "ถูกระงับ" → ปล่อยผ่าน** (ไม่เปลี่ยนพฤติกรรมเดิม) ·
  ไม่มีช่องโหว่เพราะเส้นทางระงับจริงเขียนค่าลงคอลัมน์เสมอ (`saveStaffHandler_` เขียนเฉพาะค่าใน
  `VALID_STATUS` · `authLine_` ตั้ง pending/active ให้แถวใหม่) แต่แถวที่เจ้าของแก้ในชีตเองแล้ว
  เผลอลบค่าทิ้ง จะไม่ถูกล็อกออกจากระบบแบบไม่มีคำอธิบาย
- ⚠️ **`me`/`logout` ต้องอยู่ใน `SESSION_EXEMPT_ACTIONS_` ต่อไป** — คนที่ถูกระงับต้องยัง "รู้ตัว"
  (frontend มีจอ `authPhase === "disabled"` ที่ขับด้วยผล `me` อยู่แล้ว) และกดออกจากระบบได้
  · **จงใจไม่ revoke session ตอนระงับ** ด้วยเหตุผลเดียวกัน — revoke แล้ว `me` คืน `invalid:true`
  → frontend เตะออกไปหน้าล็อกอินเปล่า ๆ ซึ่งบอกอะไรไม่ได้เลยว่าทำไมเข้าไม่ได้
- ⚠️ **`handleOrder_` ต้องตัดสินใจ "ก่อน" คว้า `ScriptLock`** — ล็อกนั้นเป็นตัวเดียวของทั้งสคริปต์
  คว้ามาแล้วค่อยปฏิเสธ = ไปกันคนอื่นที่สั่งของถูกต้องอยู่โดยเปล่าประโยชน์
- เทสต์: `tests/session-status-gate.test.js` (19 เคส — eval ฟังก์ชันจริงจาก `.gs` ไม่ copy
  เหมือน auth.test.js) · ผ่าน mutation test แล้ว (ถอดด่านออก → แดง 6 เคส)

⚠️ **`loadAuth` ใน `auth.test.js` เคยได้ `sessionInactiveOrNull_` มา "โดยบังเอิญ"** — regex ของ
`COMMON_ACTIONS_`/`MTO_JOB_ACTIONS_` ใช้ `\n *\];` ซึ่งไม่ non-greedy พอ เลยกวาดโค้ดยาวเกิน
~10KB ติดมาด้วย · เติม `grab()` ของฟังก์ชันนี้เข้าไปตรง ๆ แล้ว — พึ่งความบังเอิญนั้นไม่ได้
ขยับโค้ดเมื่อไหร่ sandbox จะขาดฟังก์ชันแล้วโยน ReferenceError ทั้งไฟล์

### F04 — เครื่องร้าน (`storedevice`) เห็นปุ่มผิด และกดรับของไม่ได้ (แก้แล้ว)

สิทธิ์ปุ่มในเส้นทางออเดอร์เดิมเขียนเป็น **blacklist** (`role !== "frontstore" && role !== "saler"`)
→ `storedevice` ที่เพิ่มเข้าระบบ**ทีหลัง** หลุดเข้ามาเห็นปุ่ม ✕ ยกเลิก / ✅ Done เอง ทั้งที่
`deleteOrder` ฝั่ง server เปิดให้แค่ `employee`/`warehouse` → **กดแล้วขึ้นแดงทุกครั้ง**
· กลับกัน `canConfirm` เป็น whitelist ที่ **ลืม storedevice** → เครื่องกลางที่ยืนอยู่หน้าร้าน
**กดรับของไม่ได้เลย** ทั้งที่ `confirmShipmentReceive` อยู่ใน `COMMON_ACTIONS_` (ทุก role ยิงได้)

- ตั้งชื่อ "การกระทำ" แทนการไล่ยกเว้น role: **`canPrepareOrder` / `canCancelOrder` /
  `canReceiveShipment` / `canEditShipment`** (views-analytics.jsx, เหนือ `OrderItemRow`)
- ⚠️ **ใช้ whitelist เสมอ ห้ามกลับไป blacklist** — role ใหม่ต้องมาตัดสินใจที่ตารางนี้
  ไม่ใช่ได้สิทธิ์ฟรีเพราะบังเอิญไม่ได้ถูกยกเว้นไว้ (มีเทสต์สแกนหา `role !== "..."` ที่หลงเหลือ)
- ⚠️ **`canCancelOrder` ต้องตรงกับ `IMMEDIATE_GATE_ACTIONS_.deleteOrder/deleteOrders` ใน `.gs`
  เป๊ะ** (`["employee","warehouse"]` + owner/dev) — มีเทสต์เทียบ 2 ไฟล์ให้แล้ว
- ⚠️ **`canReceiveShipment` ไม่ได้ล้อตาราง `.gs`** — `confirmShipmentReceive` เปิดให้ทุก role
  อยู่แล้ว ตัวนี้จึงเป็น "ใครมีหน้าที่กดรับของ" ตามงานจริง ไม่ใช่ขอบเขตสิทธิ์
- `storedevice` **ไม่เห็นปุ่มจัดของ/Done** — เท่า `saler` ตามที่ CLAUDE.md ระบุไว้แต่แรกว่า
  "สิทธิ์ API เท่า saler ทุกอย่าง" (เครื่องกลางหน้าร้านไม่ใช่คนจัดของ)

### F06 — ปุ่มเปลี่ยนสถานะทันทีแต่ไม่ตรวจว่าบันทึกผ่าน (แก้แล้ว)

`setPrintFlag` (PRINT/SKIP) · `setCarryMode` (หิ้ว↔ขึ้นรถ) · `setToCentral` · `undoComplete`
ยิง `syncOrderUpdate` แล้ว **ไม่เคยอ่านคำตอบเลย** — บทเรียนข้อ 13 ตรง ๆ (GAS ตอบหน้า HTML ได้
เมื่อ execution ซ้อนกัน) → จอเครื่องนี้เปลี่ยนแล้ว แต่ชีตยังเป็นค่าเดิม → เครื่องอื่นเห็นคนละอย่าง
งานถูกส่งต่อผิด **โดยไม่มี error ให้เห็น** · `savePrepQty`/`markComplete` ในไฟล์เดียวกันบรรทัด
ติดกันทำถูกอยู่แล้ว — เอารูปแบบเดิมมาใช้ ไม่คิดใหม่

- **`saveOrderField(updates, prevUpdates, label)`** = ตัวกลางตัวเดียวของทั้ง 4 ปุ่ม
- ⚠️ **ถอย optimistic patch กลับเมื่อล้มเหลว** — "ปุ่มเด้งกลับที่เดิม" คือสัญญาณที่ผู้ใช้เห็นได้จริง
  และคงอยู่ ต่างจาก toast ที่หายไปใน 8 วิ · ปล่อยไว้ = จอโกหกว่าเปลี่ยนแล้วจนกว่าจะ refetch
- ⚠️ **ไม่แตะ `saveFailed`** — ธงนั้นเป็นของช่อง "จัด" (ขอบแดง + ป้าย "ยังไม่บันทึก")
  ใช้ร่วมกันเมื่อไหร่ = ปุ่ม PRINT ที่บันทึกผ่านจะไปล้างคำเตือนของจำนวนที่ยังไม่เข้าระบบจริง
- ⚠️ **ห้ามยิงซ้ำอัตโนมัติ** — `updateOrderState` ยังไม่ idempotent (ไม่มี `cid` เหมือน `action=order`)
- `undoComplete` ขึ้น "ย้อนกลับแล้ว" **เฉพาะตอนบันทึกผ่านจริง**

เทสต์: `tests/order-action-roles.test.js` (16 เคส — eval ตารางจริงจาก `.jsx` + เทียบกับตาราง
`.gs` ไม่ copy) + browser test 3 เคส `ปุ่มออเดอร์/รับของ (storedevice/frontstore/warehouse)`
ที่ยืนยันบนเบราว์เซอร์จริงว่าปุ่มที่เห็นตรงกับสิ่งที่ทำได้ — **unit test เห็นแค่ค่าที่ฟังก์ชันคืน
...
### F07 — กดลงเวลาตอนคว้าล็อกไม่ได้ แล้วยังเขียนแถวต่อ (แก้แล้ว)

`punchHandler_` จับ error ของ `lock.waitLock(10000)` ทิ้ง (`catch (e) {}`) แล้ว **appendRow ต่อ** —
`_lkOk` ถูกใช้แค่ส่งเข้า `perfLock_` เพื่อวัดผลเท่านั้น ไม่เคยตัดสินใจอะไร · ล็อกตัวนี้เป็น
**สิ่งเดียว** ที่กันสองเครื่องกดพร้อมกันแล้วได้ 2 แถว → ตอนเปิดงานพร้อมกัน (จังหวะที่คนกดพร้อมกัน
มากที่สุดของวัน) การกันซ้ำหายไปเงียบ ๆ **ชั่วโมงทำงานเพี้ยนโดยไม่มี error ให้ใครเห็น**

- ตอนนี้ `if (!_lkOk)` → คืน `{success:false, retryable:true, locked:true}` **ไม่เขียนอะไรเลย**
- ⚠️ **ต้องตัดก่อน `saveAttPhoto_`** — บรรทัดนั้นเขียนไฟล์ลง Drive จริง ตัดทีหลัง = เหลือรูปค้าง
  ที่ไม่มีแถวไหนอ้างถึง (ถูกลบก็ต่อเมื่อครบ `ATT_PHOTO_KEEP_DAYS` 90 วัน)
- ⚠️ **ข้อความต้องบอกให้ "ดูสถานะล่าสุดก่อน" ไม่ใช่ "ลองใหม่" เฉย ๆ** — คำขอที่ถูกตัดยังไม่เขียน
  ก็จริง แต่คำขอ**ของคนที่ถือล็อกอยู่**อาจเพิ่งเขียนสำเร็จ · ธง `locked:true` ทำให้ frontend
  สั่ง `load()` ดึงสถานะจริงมาโชว์ทันที
- ⚠️ **ฝั่ง `doPunch` (views-attendance.jsx) เดิมขึ้น "บันทึกไม่สำเร็จ" ตรง ๆ ใน catch** ซึ่งผิด
  บทเรียนข้อ 13 — GAS เขียนแถวเสร็จแล้วยังตอบหน้า HTML/ถูกตัดที่ 90 วิได้ · ตอนนี้ขึ้น
  **"ยังตรวจผลไม่ได้ … ดูรายการด้านล่างว่าลงเวลาไปแล้วหรือยัง ก่อนกดซ้ำ"** แล้ว `load()` ทันที
- ⚠️ **ห้าม auto-retry การ punch** (เขียนข้อมูลจริง ยังไม่ idempotent — ไม่มี `cid` เหมือน `action=order`)
- ยังไม่ทำ: `saveAttPhoto_`/`readAttEvents_` ยังอยู่ในช่วงถือ ScriptLock (ล็อกตัวเดียวของทั้งสคริปต์)
  = อัปโหลด Drive คร่อมล็อกไปกันคนสั่งของ — ย้ายออกได้แต่ต้องกันไฟล์ค้างก่อน
- เทสต์: `tests/punch-lock.test.js` (15 เคส — **รัน `punchHandler_` จริงจาก `.gs`** กับ LockService
  ปลอมที่คว้าไม่ได้ แล้วนับว่า appendRow/saveAttPhoto_/writeAuditLog_ ถูกเรียกกี่ครั้ง) ·
  ผ่าน mutation test แล้ว (ถอด guard ออก → แดง 8 เคส)

### F05 — "พิมพ์แล้ว" ไม่ได้แปลว่าป้ายออกจากเครื่องพิมพ์ (แก้แล้ว)

`printOrders` (OrderSummaryView) ตั้ง `localStorage` + `printFlag="printed"` **ทันทีหลัง**
`onPrintRequest(...)` โดยไม่รอผล `syncOrderUpdate` → 2 อาการที่ไม่มี error ให้เห็นเลย
① แค่เปิดหน้าพิมพ์แล้วกดย้อนกลับ (กระดาษหมด/เลือกผิดใบ/เครื่องพิมพ์ไม่ติด) ก็ขึ้น "✓ พิมพ์ Label แล้ว"
ค้างถาวร → พนักงานข้ามงานติดป้าย ② server บันทึกไม่สำเร็จ (GAS ตอบหน้า HTML) → เครื่องนี้ขึ้น
พิมพ์แล้ว แต่เครื่องอื่นยังเห็น "ยังไม่พิมพ์" = **คนละความจริงบนสองจอ**

- **แยกเป็น 2 ขั้น**: `printOrders` = เปิดงานพิมพ์ + จำไว้ว่า "รอยืนยัน" (`awaitPrint`) เท่านั้น ·
  `confirmPrinted` = ผู้ใช้กดยืนยันเองว่าป้ายออกมาแล้ว → ตรงนี้ที่เดียวที่ **await** ผลจริงแล้วบันทึก
- ⚠️ **`awaitPrint` ผูกกับ `orderSig(o)` ไม่ใช่ `order.id` เปล่า ๆ** — เลขแถวถูก reuse ได้หลังมีคน
  กดยกเลิกใบอื่น (บั๊ก row-shift) → คิว "รอยืนยัน" ของใบเก่าจะไปโผล่บนใบใหม่ที่ไม่เคยสั่งพิมพ์
- ⚠️ **ที่บันทึกล้มเหลวต้องคงอยู่ในคิว** (ลบเฉพาะ `okIds`) — ลบทั้งชุด = หายจากจอทั้งที่ยังไม่ได้บันทึก
- ⚠️ **ไม่เก็บ `awaitPrint` ลง localStorage โดยตั้งใจ** — ปิดแอปเปิดใหม่ควรกลับไปเป็น "ยังไม่พิมพ์"
  ซึ่งปลอดภัยกว่าค้างสถานะกลางคันที่ไม่มีใครรู้ว่าจริงหรือเปล่า
- **ยังพิมพ์ซ้ำได้เสมอ** — `printableOrders` ไม่ตัดใบที่อยู่ในคิวรอยืนยันออก
- **ขอบเขต**: สถานะนี้ผูกกับ **ใบสั่งของ** ไม่ใช่ตัวสินค้า · หน้า "พิมพ์ Label" ที่เลือก SKU เองไม่แตะ
  สถานะใบสั่งของเลย → ติดข้อความบอกไว้บนหน้านั้น (ไม่บอก = พนักงานนึกว่าพิมพ์จากที่ไหนก็ตัดงานให้)
- ⚠️ **ห้ามยิงซ้ำอัตโนมัติ** — `updateOrderState` ยังไม่ idempotent
- **ป้าย "พิมพ์แล้ว" ต้องเห็นชัดโดยไม่ต้องอ่านตัวหนังสือ (เจ้าของขอ ส.ค. 2026)** — เดิมเป็นข้อความ
  เปล่า ๆ สีเขียว ✓ เล็ก ๆ ตอนนี้ทำเป็น "ป้าย" มีพื้นหลัง+กรอบเขียว (`#e8f5e9`/`#4fb472`) ใส่
  🖨️ นำหน้า + เครื่องหมายถูกตัวใหญ่ (`fontWeight:900`) แยกจากตัวหนังสือ — พนักงานกวาดตาดูรูปการ์ด
  เจอสีเขียว+ไอคอนเครื่องพิมพ์ก็รู้ทันทีว่าใบนี้จบงานพิมพ์แล้ว ไม่ต้องอ่านคำ
- เทสต์: `tests/print-confirm.test.js` (18 เคส — ตัดฟังก์ชันจริงจาก `.jsx` ไม่ copy · เพิ่มเคส
  บังคับว่าป้ายมี 🖨️ + fontWeight:900 + พื้นหลังสี ไม่ใช่แค่ข้อความลอย)

### F01 — สิทธิ์เจ้าของถูกให้จาก "ชื่อที่โชว์ใน LINE" (แก้แล้ว)

`authLine_` เทียบ `AUTO_OWNER_LINE_NAMES = ["tah","jeed"]` กับ **ชื่อที่โชว์ใน LINE ซึ่งใครก็
เปลี่ยนเองได้** แล้วให้ `owner`/`active` ทันที ทั้งแถวใหม่และแถวเดิม → คนนอกตั้งชื่อ LINE ว่า "tah"
แล้วล็อกอิน = ได้ owner · และ **แถวที่เจ้าของกดระงับไว้กลับเป็น active เอง** ทำให้ขั้นตอน
"อนุมัติ/ระงับ" ที่สอนเจ้าของไปเชื่อถือไม่ได้เลย

- **เส้นทางถาวร (ตัวหลัก)**: Script Property **`AUTO_OWNER_LINE_IDS`** = `providerUserId` ของ
  LINE คั่นด้วย `,`/เว้นวรรค/ขึ้นบรรทัด — **รหัสถาวร เปลี่ยนเองไม่ได้** และตั้งได้เฉพาะคนที่เข้า
  GAS Script Properties (= เจ้าของ) · ให้ owner/active เสมอ = **ช่องทางกู้บัญชีเจ้าของ**
  · เจ้าของหา id ตัวเองได้ด้วย **`listStaffLineIds()`** (GAS editor · ชื่อไม่มี `_` ต่อท้ายจึงโผล่
  ใน dropdown · **อ่านอย่างเดียว**) แล้วก๊อปไปใส่ property
- **เส้นทางชื่อ (ทางกู้ที่แคบลง)** ต้องผ่าน **ทั้ง 2 ข้อ**:
  · **ชื่อนั้นยังไม่ถูกบัญชีอื่นใช้อยู่** (`autoOwnerNameTakenBy_` เทียบทั้ง col D และ E) —
    คนที่ 2 ที่ใช้ชื่อซ้ำไม่ได้สิทธิ์ ตกไปรออนุมัติตามปกติ
  · **แถวเดิมต้องเป็น `pending` หรือ status ว่าง เท่านั้น** (`canUpgrade`) — สองค่านี้ = ยังไม่มี
    มนุษย์ตัดสินใจอะไรกับแถวนี้ · **`disabled` คือการตัดสินใจของเจ้าของ ห้ามเปิดกลับด้วยชื่อ
    เด็ดขาด** · `active` ที่ role อื่นก็ไม่ทับ (เจ้าของตั้งเอง)
    · ⚠️ **นับ status ว่างเข้ามาด้วยโดยตั้งใจ** — เส้นทางเขียนจริงใส่ค่าเสมอ (`authLine_` ใส่
      active/pending · `saveStaffHandler_` ใส่เฉพาะค่าใน `VALID_STATUS`) ว่างได้ทางเดียวคือมีคน
      ลบค่าในชีตเอง · ไม่นับรวม = **เจ้าของที่แถวโดนลบค่ายกระดับตัวเองกลับไม่ได้เลย**
- ⚠️ **ปฏิเสธแล้วห้ามเงียบ** — ชื่อซ้ำ → audit + แจ้ง LINE เจ้าของพร้อม **บอก `providerUserId`
  ให้เอาไปใส่ property** (เผื่อเป็นเจ้าของที่เปลี่ยนบัญชี LINE จริง) · แถวที่ถูกระงับ → เขียน audit
  · ไม่บอก = คนที่โดนปฏิเสธกลายเป็น "ล็อกอินแล้วไม่ได้สิทธิ์ ไม่มีใครรู้ว่าทำไม"
- ⚠️ **ยกระดับแล้วต้อง `sessionCacheClearForStaff_`** ไม่งั้นถือสิทธิ์เก่าต่ออีก 5 นาที
- ⚠️ **ห้าม hard-code รหัส LINE ของเจ้าของลงไฟล์ที่ push** — อยู่ใน Script Property เท่านั้น (มีเทสต์)
- **เจ้าของเดิมที่ `owner`/`active` อยู่แล้วไม่ถูกแตะเลย** (เกณฑ์ "เจ้าของเดิมยังเข้าได้")
- เทสต์: `tests/auto-owner.test.js` (22 เคส — eval ฟังก์ชันจริงจาก `.gs` + ชีตจำลอง ไม่ copy)

### F08 — งานขายที่ยังไม่บันทึกหายเมื่อเปลี่ยนเมนู (แก้แล้ว)

`PosView` เก็บ cart/ลูกค้า/จัดส่งใน `useState` ล้วน และ App mount เฉพาะ `activeTab` → เซลกรอก
หลายสินค้าแล้วไปดูสต๊อกอีกแท็บ กลับมาต้องกรอกใหม่ทั้งใบ (มือถือสลับงานบ่อยมาก)

- เก็บ snapshot ลง `localStorage.dmj_pos_draft_v1` ทุกครั้งที่ตะกร้า/ลูกค้า/จัดส่งเปลี่ยน ·
  ตะกร้าว่าง/ออกบิลแล้ว (`result`) = ลบทิ้ง · TTL 12 ชม. (เกินนั้น = คนละกะ)
- ⚠️ **ห้ามกู้คืนอัตโนมัติเด็ดขาด** — เครื่องกลาง (`storedevice`) เป็นบัญชี LINE เดียวที่หลายคนใช้
  ผูก staffId แล้วก็ยังแยกคนไม่ได้ · กู้เองเมื่อไหร่ = **บิลของคนก่อนโผล่บนจอคนถัดไป**
  → แสดงเป็นแบนเนอร์ "↩️ มีบิลที่กรอกค้างไว้ N รายการ · เวลา · โดยใคร" ให้กด **กู้/ทิ้ง** เอง
  (เกณฑ์ปิดงาน "ไม่มี draft ของคนก่อนหลงมาบนเครื่องกลาง")
- ⚠️ **กู้โหมด/ช่องทาง/วิธีชำระเป็นชุดเดียวกัน** — กู้ครึ่ง ๆ = "เงินสด" ค้างมาในโหมดออนไลน์
  แล้วบันทึกรับชำระทั้งที่เงินยังไม่เข้า (กับดักเดียวกับตอนสลับโหมด)
- `localStorage` เต็ม → `posDraftWrite` กลืน error เงียบ ๆ (เก็บ draft ไม่ได้ ต้องไม่ทำให้ขายไม่ได้)

### F09 — พิกัดที่ส่งตอนลงเวลาอาจเป็นค่าของ "ตอนเปิดหน้า" (แก้แล้ว)

`doPunch` คอมเมนต์เขียนว่า "ขอพิกัดสด ณ ตอนกด" แต่โค้ดจริงหยิบค่าที่ขอไว้ **ตอนเปิดหน้า** มาใช้เลย
เมื่อ `gps.state === "ok"` → ค้างหน้าลงเวลาไว้แล้วเดินไปอีกจุด กดปุ่มได้พิกัดของจุดเดิม =
**ระยะห่าง/ชื่อจุดในชีตผิด โดยไม่มี error ให้เห็น** (ข้อมูลนี้ใช้ยืนยันว่ามาถึงที่ทำงานจริง)

- `attGetPosition` ติด `at` (เวลาที่อ่านได้) มาด้วยเสมอ · `doPunch` ใช้ค่าที่ cache ไว้ได้ไม่เกิน
  **`ATT_GPS_MAX_AGE_MS` (90 วิ)** เกินนั้นขอใหม่ แล้ว `setGps` ให้จอตรงกับที่ส่งไปจริง
- หน้าจอขึ้น "⏳ พิกัดนี้อ่านไว้เมื่อ N นาทีที่แล้ว — กดลงเวลาแล้วระบบจะขอตำแหน่งใหม่ให้"
- ข้อความตอนอ่านคำตอบไม่ได้ แยก **"ยังตรวจผลไม่ได้"** ออกจาก **"ปฏิเสธแล้ว"** (ดู F07)
- เทสต์: `tests/pos-draft-gps.test.js` (20 เคส — F08 + F09 · รันตัวจัดการ draft จริงจาก `.jsx`)

### ยังไม่ทำ — งานที่เหลือจากรายงาน

| | เรื่อง | ทำไมยังไม่ทำ |
|---|---|---|
| **F11** | browser test ไม่ได้ใช้ CSS จริง — `harness.html` มี `<style>` เล็ก ๆ ของตัวเอง ไม่ได้โหลด CSS จาก `Doomuenjing Dashboard.html` | แก้ได้ (ให้ static server ใน `run.cjs` เสิร์ฟ `<style>` จากไฟล์จริงเป็น `/tests/browser/prod.css`) แต่เป็นการเปลี่ยน **ตัวเทสต์เอง** ซึ่งอาจทำให้เคสเดิมแดงยกแผง — **แยกเป็นก้อนของตัวเอง ห้ามขึ้นพร้อมก้อนแก้บั๊ก** ไม่งั้นแยกไม่ออกว่าอะไรพัง |
| **F10 · F13** | คู่มือหลายฉบับเก่า/ไม่มีทางเข้าในเมนู · ต้องมีพจนานุกรมนิยามตัวเลข | เป็นงาน **เขียนเนื้อหา + ตรวจรับกับเจ้าของ** ไม่ใช่งานโค้ด · การเพิ่มเมนูชี้ไปคู่มือเก่าที่ยังผิดอยู่ **แย่กว่าไม่มีเมนู** |
| **F12** | ภาษาพนักงาน/ปุ่มย่อยต้องตรวจหน้างาน | ต้องดูของจริงบนมือถือกับพนักงาน — เซสชันนี้ทำแทนไม่ได้ |
| **F14** | โครงสร้างใหญ่/กฎซ้ำ | งานจัดระเบียบระยะยาว ทำหลังแกนงานนิ่ง |
...
**แนวนอน** = ตาราง · มือถือ **แนวตั้ง** = การ์ด · เดิมการ์ดมีปุ่มพิมพ์ปุ่มเดียวที่เรียก
`handlePrint(q)` **ไม่ส่ง `docType`** → ตกเป็น `"quotation"` เสมอ = **พิมพ์ใบแจ้งหนี้จากมือถือ
ไม่ได้เลย** ทั้งที่หมุนจอเป็นแนวนอนแล้วยังกดได้ (เจ้าของแจ้งเอง — หน้าจอดูปกติทุกประการ
ไม่มี error ให้เห็น มีแค่ "ปุ่มหาย")
- **แก้ view ไหนที่มี `{mobile ? (การ์ด) : (ตาราง)}` ต้องไล่ดูให้ครบทั้งสองฝั่งเสมอ** — ปุ่มที่
  เพิ่มในตารางอย่างเดียวจะไม่มีวันโผล่บนมือถือ ซึ่งเป็นจอหลักของร้าน
- ปุ่มพิมพ์บนการ์ดใช้ **ป้ายชื่อเอกสาร** (`🖨️ ใบเสนอราคา` / `🧾 ใบแจ้งหนี้`) ไม่ใช่ไอคอนเปล่า —
  บนจอเล็ก 🖨️ กับ 🧾 แยกไม่ออกว่าอันไหนคืออะไร · วางเป็นแถวของตัวเอง (ตรวจแล้วไม่ล้นที่ 320px)
- เทสต์: `tests/quote-print-mobile.test.js` (11 เคส — ตัดบล็อก `{mobile ? (` จาก `.jsx` จริง
  แล้วบังคับว่าทั้ง 2 โหมด (รออนุมัติ/อนุมัติแล้ว) มีปุ่มครบ + ห้ามมี `handlePrint(q)` แบบไม่ระบุชนิด)

### 👤 แท็บ "ใบเสนอราคา" — หน้าเจ้าของ ≠ หน้าพนักงานขาย (ส.ค. 2026)

เจ้าของสั่ง: พนักงานขายควรเห็น **หน้าทำงาน** (สร้าง/ตาม/พิมพ์ของตัวเอง) ส่วนเจ้าของเห็น **หน้าติดตามผล**
· เดิม `QuoteFollowupView` เรนเดอร์เหมือนกันทุก role — saler/storedevice เปิดมาเจอแดชบอร์ดผู้บริหาร
("📊 สรุปสถานะ" + KPI อัตราอนุมัติ + "👤 ตามเซล") ก่อนเสมอ กว่าจะสร้างใบต้องไปกดปุ่มเล็กมุมขวา

- **แยกด้วย `isOwner = role === "owner"`** (viewRole ยุบ dev→owner มาแล้ว) — **frontend ล้วน ไม่แตะ
  `.gs`/`ROLE_ACTIONS_`** เพราะทุก role ยิง `getQuotationSummary` เห็นข้อมูลชุดเดียวกันอยู่แล้ว เป็นแค่
  การ *เลือกโชว์* ไม่ใช่สิทธิ์
- **เจ้าของ (owner/dev)**: คงเดิมทุกอย่าง — ตัวกรองปี/เดือน · KPI 5 ช่อง · "👤 ตามเซล" (เทียบ %ปิด
  รายคน) · ตารางรายเดือน · รออนุมัติ/อนุมัติแล้ว (ทุกคน) · default `mode="summary"`
- **พนักงานขาย (saler/storedevice)**: default `mode="pending"` (เข้าหน้าตามงานเลย) · ปุ่ม
  **"➕ สร้างใบเสนอราคาใหม่" เต็มความกว้าง** · ชิป **"⭐ ของฉัน / 📋 ทั้งหมด"** · ไทล์ 3 ช่อง
  (รออนุมัติ/อนุมัติแล้ว/ทั้งหมด) · แค่ 2 โหมด (รออนุมัติ/อนุมัติแล้ว) · **ซ่อน** แดชบอร์ด/ตามเซล/
  อัตราอนุมัติ/ตัวกรองปี-เดือน/ช่อง "+ ชื่อเซล" ทั้งหมด
- ⚠️ **"ของฉัน" match ด้วย `it.sale === window._currentUserName`** (ชื่อล้วน) — `it.sale` มาจาก tag
  ZORT ที่ `createQuotation` ประทับด้วยชื่อ session ตอนสร้าง (`payload.tag=[salesRep]`, salesRep ถูก
  doPost ทับด้วย session displayName) → มาจากที่เดียวกัน · **ห้ามเทียบกับ `window._currentUser`**
  ที่มี "(ตำแหน่ง)" ต่อท้าย = ไม่ match เลยสักใบ
- ⚠️ **ห้าม hard-restrict "ของฉัน"** — ใบที่สร้างใน ZORT UI/ใบเก่าไม่ติด tag → `sale` ว่าง →
  หายจาก "ของฉัน" · saler หน้าเคาน์เตอร์อาจต้องพิมพ์ซ้ำใบเพื่อน → มีปุ่ม "📋 ทั้งหมด" เสมอ
  (default: saler = ของฉัน · storedevice = เครื่องกลางใช้ร่วมกัน → ทั้งหมด)
- ⚠️ **onBack จากโหมด create ต้อง `setMode(isOwner ? "summary" : "pending")`** — คืนเป็น "summary"
  เฉย ๆ = พนักงานกดสร้างแล้วย้อนกลับไปเจอบล็อกแดชบอร์ดที่ถูกซ่อน (จอว่าง)
- **รายการฝั่งพนักงานไม่ผูกปี/เดือน** (`empPending`/`empApproved` ผ่าน `scopeMine`) — ตามงานของตัวเอง
  ทั้งหมด · เจ้าของยังใช้ `pendingList`/`approvedList` (ผูกปี/เดือน) · call site เลือกด้วย
  `pendingRender`/`approvedRender = isOwner ? … : …`
- อนุมัติ/ปิดใบ/แก้ไข ยังกดได้ทั้ง 2 ฝั่ง (saler อยู่ใน `IMMEDIATE_GATE_ACTIONS_` ของ approve/void/edit อยู่แล้ว)
- เทสต์: `tests/browser/run.cjs` interaction 2 เคส (owner เห็นแดชบอร์ด+ตามเซล+อัตราอนุมัติ · saler
  จอ 420px เห็นปุ่มสร้างใหญ่+ชิปของฉัน, เห็นเฉพาะใบตัวเอง, กด "ทั้งหมด" ถึงเห็นใบเซลอื่น, ไม่มี
  เครื่องมือผู้บริหาร) + `__DMJ_QUOTE_FIXTURE` (fixture.js) ที่ `sale` ของ "ของฉัน" = `'ทดสอบ ระบบ'`
  ให้ตรงกับชื่อที่ harness `me` คืน — ไม่ตรง = ชิปของฉันว่าง แล้วเทสต์เขียวลวง · harness ต้องมี branch
  `action=getQuotationSummary` ก่อน branch payload รวม (ไม่งั้นได้ payload สินค้า → items ว่าง → "ไม่มีใบเสนอราคา")

### 🎁 หมวด Made to Order ในหน้าสร้างใบเสนอราคา — ยุบเป็น "การ์ดกลุ่ม" (ส.ค. 2026)

เจ้าของขอ: หมวด MTO ("Made to Order/จัดแบบพิเศษ") มีรหัสเป็นชุด (BK001-BK020, PACKAGE001-…)
หลายร้อยตัว เลื่อนหา 14 หน้าไม่ไหว → ยุบเป็น **การ์ดกลุ่มเดียวต่อ prefix** กดแล้วเพิ่มรหัสถัดไปที่ยัง
ว่าง (BK001→002→003) เป็น **บรรทัดแยก** ในใบเสนอราคา · ทำ **เฉพาะหมวด MTO** (`isMtoCat(catFilter)`)
หมวดอื่นยังโชว์รายตัวเหมือนเดิม

- **`mtoSkuPrefix_(sku)`** = ตัวอักษรอังกฤษล้วนที่นำหน้า**ตัวเลข** (`BK001`→`BK`, `PACKAGE001`→`PACKAGE`)
  · ไม่เข้ารูปแบบ (รหัสไม่มีเลข/ชื่อล้วน) → คืน `''` = **กลุ่มเดี่ยวของตัวเอง** (key = `'#'+sku`)
  **ห้ามยุบมั่ว** ไม่งั้นของที่รหัสไม่เข้าแพทเทิร์นหายจากจอเงียบ ๆ
- **`mtoGroupProducts_(list)`** เรียงรหัสในกลุ่มแบบ **numeric** (`localeCompare(..,{numeric:true})`) →
  BK002 มาก่อน BK010 · กดการ์ด = `addNextInGroup` หยิบ `items.find(!inCart)` = ตัวถัดไปที่ยังไม่อยู่ในใบ
  (กด 3 ครั้งได้ 3 รหัสไล่กัน ไม่ใช่เพิ่มตัวเดิมซ้ำ)
- ยังกด **"เลือกแบบเอง"** เพื่อกางเลือกรหัสเจาะจงได้ (เผื่อลูกค้าสั่งลายเฉพาะ) — `expandGroup` state ·
  ตัวที่อยู่ในใบแล้ว disable + ติ๊ก ✓
- **frontend ล้วน ไม่แตะ `.gs`** — เป็นแค่การจัดกลุ่มการแสดงผลของ picker เดิม (ราคา/ส่วนลด/สต็อก
...
เว็บอ่านสินค้าจากชีต (`SHEET_PRODUCT_META` + SELF-HEAL จาก `SHEET_PRODUCTS`) **ไม่ได้อ่านสด ZORT** ·
`syncZortBoth` แค่ทับ "ตัวเลข" ตาม SKU **ไม่เคยลบแถว** → สินค้าที่ลบใน ZORT ค้างโชว์ตลอดไป

- **ชีตใหม่ `SHEET_HIDDEN_PRODUCTS` = "สินค้าที่ซ่อน"** (A=sku B=reason C=hiddenAt D=source) —
  **soft-delete ไม่ลบแถวจริง กู้กลับได้** · `readProducts_` กรอง SKU ในชีตนี้ออก **ทั้ง 2 ลูป**
  (main + SELF-HEAL) ผ่าน `getHiddenSkuSet_()` → สินค้าที่ซ่อนหายจากเว็บ
- ⚠️ **กับดักร้ายแรง**: ดึง ZORT ไม่ครบ (เน็ตพัง/quota) แล้วเหมาว่า "ไม่เจอ = ถูกลบ" =
  **ซ่อนทั้งร้านเงียบ ๆ** · มี 2 ด่าน: (1) **`zortAllSkusComplete_()`** หน้าใดพังหลัง retry →
  `ok:false` **ไม่คืน partial** (ต่างจาก `fetchAllZortProducts_` ที่คืน partial เงียบ ๆ) ·
  (2) **`HIDE_SAFETY_FRACTION = 0.4`** — จะซ่อนเกิน 40% ของสินค้าทั้งหมด → **abort** ให้ตรวจก่อน
- **`zortDeletedSkusCore_(ourSkuMap, zortSet, hiddenSet)`** = pure diff (เทสต์ได้) — SKU ที่ชีตเรามี
  แต่ ZORT ไม่มี และยังไม่ถูกซ่อน · เทียบแบบ uppercase · เก็บทั้ง `sku` และ `barcode` ของ ZORT เข้า set
  (SKU เรา = barcode ZORT ในระบบนี้) กัน false-positive
- **เครื่องมือรันใน GAS editor** (ชื่อไม่มี `_` → โผล่ใน dropdown, บทเรียนข้อ 1):
  · **`previewZortDeletedProducts()`** — อ่านอย่างเดียว บอกว่าจะซ่อนตัวไหน (รันดูก่อนเสมอ)
  · **`hideDeletedFromZort()`** — เขียนจริง (soft-delete) + จับ `LockService` + `invalidateCache_()`
  · **`listHiddenProducts()` / `unhideProduct("SKU")` / `clearHiddenProducts()`** — ดู/กู้กลับ
- เทสต์: `tests/zort-deleted.test.js` (12 เคส — eval `zortDeletedSkusCore_` จริงจาก `.gs` +
  meta-test คุมด่านความปลอดภัย: abort เมื่อดึงไม่ครบ, เพดาน %, readProducts_ กรอง 2 ลูป, กู้กลับได้)
...
- **ยืนยันจากหน้า ZORT แล้วว่า `pricepernumber × number` คือตัวที่ ZORT ใช้คิดยอดต่อบรรทัดจริง**
  (ไม่ใช่ field `totalprice` ที่เราส่งไป — ถ้าไม่ส่ง `pricepernumber` ยอดรวมเป็น 0 ทันที)
- ⚠️ **หมายเหตุเดิมตรงนี้เคยเขียนผิด** ว่า "AddOrder ไม่มี field ยอดรวมหัวเอกสารให้ส่งเอง จึงมาจาก
  ผลรวมบรรทัดล้วน ๆ" — **ไม่จริง** (เป็นการอนุมาน ไม่ได้ยืนยัน) ของจริงคือ AddOrder ต้องส่ง
  `amount`/`amount_pretax`/`vatamount` เองเหมือน AddQuotation ดูหัวข้อถัดไป
...
### 🔴 "ออกบิลสำเร็จ แต่ ZORT ขึ้นมูลค่า 0" — ยอดหัวเอกสารต้องส่งเอง (ส.ค. 2026)

เจอจริง: บิล **RC-202608007 ยอด ฿476** ออกสำเร็จ แอปโชว์ ฿476 ครบ แต่หน้า "รายการขาย" ของ ZORT
ขึ้น **มูลค่า `0`** และ **ช่องทาง `–`** → **ยอดขายไม่เข้าระบบ ZORT เลย** (รายงานยอดขาย/สต็อกเพี้ยนตาม)

- **ต้นเหตุ**: `AddOrder` **ไม่คำนวณยอดรวมหัวเอกสารจาก `list[].totalprice` ให้เอง** — พฤติกรรม
  เดียวกับ `AddQuotation` เป๊ะ ซึ่งเคยเจอและยืนยันไปแล้วด้วย `exploreZortQuotationAmountFix()`
  (test-then-void) ว่า "ไม่ส่ง = ขึ้น 0 ในหน้า ZORT ทั้งที่ line item ถูกต้อง"
  · `createQuotation` แก้ไปตั้งนานแล้ว แต่ **`createSaleBill` ไม่เคยแก้ตาม** — หลุดมาตลอด
- **แก้**: ส่ง `amount`/`amount_pretax`/`vatamount` ที่ header ผ่าน **`zortOrderAmounts_()`**
  (ตัวเดียว ใช้ทั้งยอดหัวเอกสารและ `paymentamount` — คิดซ้ำ 2 ที่ = ZORT ขึ้นรับเงินเกิน/ขาด)
- ⚠️ **ห้ามส่ง `vattype`/`vatpercent`/`discount` ปนไปด้วย** — ลองกับ AddQuotation แล้ว ZORT
  จะ recompute `amount_pretax`/`vatamount` ทับเป็นค่าผิด **แย่กว่าไม่ส่งเลย** (มีเทสต์กันไว้)
- **ช่องทางขายก็หลุดด้วยเหตุเดียวกัน**: `POS_ZORT_FIELDS.orderChannel` เคยเป็น `"channel"` ซึ่ง
  ZORT ไม่รู้จัก → ทุกบิลขึ้นช่องทาง `–` · เปลี่ยนเป็น **`"saleschannel"`** ให้ตรงกับ
  `createQuotation` ที่เวิร์กจริง
- ⚠️ **บทเรียนที่ใหญ่กว่าตัวบั๊ก**: `POS_ZORT_FIELDS` ของฝั่ง order ถูกทำเครื่องหมายไว้ตั้งแต่ต้นว่า
  "ยัง**อนุมาน**ตาม ZORT API v4" แต่ไม่เคยมีใครไปยืนยันจริง · **ฟิลด์ที่เดาแล้ว ZORT เงียบ ๆ ไม่
  บ่นเลย** (ไม่ error ไม่เตือน แค่ไม่บันทึกค่า) — เจอได้ทางเดียวคือเปิดหน้า ZORT ดูของจริง
- **เครื่องมือตรวจ**: `checkSaleBillInZort("RC-...")` (GAS editor · ชื่อไม่มี `_` ต่อท้ายจึงโผล่ใน
  dropdown · **อ่านอย่างเดียว**) — ดึง order จริงมากางให้เห็นว่า ZORT เก็บอะไรไว้ แล้วสรุปให้เลยว่า
  เป็นกรณี **(ก) line item ไม่มีราคา** หรือ **(ข) line item ถูกแต่ยอดหัวเอกสาร 0** ซึ่งแก้คนละทาง
  · เทียบกับชีต "บิลขาย" ฝั่งเราให้ด้วยว่ายอดตรงกันไหม
...
เทสต์: `tests/online-sale.test.js` (72 เคส — eval ฟังก์ชันจริงจาก `.jsx` + `.gs` ไม่ copy
...
### ⭐ saler + storedevice นับหน้าร้านได้ แต่ **ไม่บังคับ** (ส.ค. 2026)

เจ้าของสั่ง: "เพิ่มปุ่มกดเช็คสินค้าให้ตำแหน่ง saler" ตามด้วย "เปิดเครื่องกลางด้วย" — ทั้งคู่ยืนอยู่
หน้าร้านเห็นชั้นวางจริง ควรนับ/บันทึกยอดได้เหมือนหน้าร้าน **แต่ห้ามบังคับ** เพราะงานหลักคือ
ปิดการขายให้ทันลูกค้าที่ยืนรออยู่ตรงหน้า — บังคับนับก่อนสั่งเมื่อไหร่ = เอางานคนอื่นไปขวางการขาย

- **ธงเดียวเดิม (`needFsCheck`) ถูกแยกเป็น 2 ตัว** ใน `OrderModal` — ของเดิมทำหน้าที่ทั้ง
  "โชว์การ์ดนับ" และ "กั้นปุ่มยืนยัน" พร้อมกัน เปิดให้ 2 role นี้เมื่อไหร่ก็โดนกั้นไปด้วยทันที
  · **`canFsCheck`** (frontstore/employee/**saler/storedevice**) = การ์ดนับโผล่ + บันทึกยอดได้
  · **`mustFsCheck`** (frontstore/employee) = กั้นปุ่มยืนยันจนกว่าจะกรอก (`fsBlocked`)
- ⚠️ **ยุบกลับเป็นตัวเดียวเมื่อไหร่ saler/storedevice กดสั่งของไม่ได้ทั้งตำแหน่ง โดยไม่มีอะไรเตือน**
- ⚠️ **`canCountFs` (ProductCard) ต้องตรงกับ `canFsCheck` (OrderModal) เสมอ** — ตัวแรกตัดสินว่า
  "ปุ่มบนการ์ดกดได้ไหมตอนของหมด" ตัวหลังตัดสินว่า "เปิดเข้าไปแล้วมีอะไรให้ทำไหม" · ไม่ตรงกัน =
  กดปุ่มเข้าไปเจอ modal ที่บอกว่าสั่งไม่ได้แต่ไม่มีอะไรให้ทำต่อ = **ทางตัน จอดูปกติ ไม่มี error**
- ⚠️ **กรอบสีเตือน (ส้ม) ผูกกับ `fsBlocked` ไม่ใช่ "ยังไม่กรอก"** — ไม่งั้น 2 role นี้เห็นกรอบส้ม
  เตือนทั้งที่กดสั่งได้ปกติ · เลข **"②"** หน้า "จำนวนที่สั่ง" ก็ขึ้นเฉพาะตอนมีขั้น ① บังคับจริง
  (โชว์ ② โดยไม่มี ① = ลำดับที่ขาดขั้นแรก อ่านแล้วงงว่าตกอะไรไป)
...
| ของโอนมาหน้าร้าน | frontstore, employee, owner | orders (view `shipped`) | ใน `transferStockBatch` · รวมทั้งชุดเป็นอันเดียว ไม่ยิงราย SKU |
| หน้าร้านรับของไม่ครบ | warehouse, owner | tracking | ใน `confirmShipmentReceive` · **รับครบไม่แจ้ง** (ไม่กวน) |
| สต็อกใกล้หมด (คลัง) | owner, warehouse | stock | dedupKey ผูกวันที่ = 1 ครั้ง/วัน แม้ตัวเช็ครันทุก 2 ชม. |
| ของหมดหน้าร้าน คลังยังมี | frontstore, saler, storedevice, employee, owner | categories (view `reorder`) | ใน `syncZortBoth` · **ไม่ยิง LINE** · ดูหัวข้อด้านล่าง |
| ของค้างรับเกิน 3 วัน | frontstore, employee | orders (view `shipped`) | ใน `archiveReceivedShipments` (ตี 3) · คนที่ **กดรับได้** |
| ของค้างรับเกิน 3 วัน | warehouse, owner | tracking | แถวแยกจากอันบน — คนที่ต้อง **ตามของ** (กดรับแทนไม่ได้) |
...
### กดแจ้งเตือน "ของโอนมาหน้าร้าน" ต้องไปหน้าที่ **กดรับของได้** (ส.ค. 2026)

เจ้าของแจ้ง: กดแจ้งเตือน "🚚 ของโอนมาหน้าร้าน N รายการ · รอกดรับ" แล้วต้องเด้งไปหน้าเช็ครับของเลย
· เดิมพาไปแท็บ **"สต๊อก"** ซึ่งดูจำนวนได้อย่างเดียว **กดรับไม่ได้** → แจ้งไปแล้วพนักงานก็ยังต้อง
ไล่หาหน้าที่ถูกเอง (หน้าที่ทำงานกับของกองนั้นจริงคือแท็บ **"รายการสั่งของ" ตัวกรอง "🚚 ส่งแล้ว"**
ที่เดียวกับที่ชิป "🚚 ของรอรับ" บนหน้าหลักพาไป — **สองทางเข้าต้องจบที่หน้าเดียวกัน**)

- **คอลัมน์ N `view`** (`INAPP_COL.VIEW=14`) — **ต่อท้าย ไม่แทรกกลาง** (บทเรียนข้อ 5) ·
  คนละเรื่องกับ `focus`: `focus` = "พาไปหาของชิ้นไหน" · `view` = "เปิดมาแล้วต้องเห็นมุมมองไหน"
  · จำเป็นเพราะเรื่องที่รวมหลาย SKU (โอนทั้งชุด) **ใส่ `focus` ไม่ได้** แต่ยังต้องพาไปให้ตรงหน้า
- **สายส่ง**: GAS col N → `listInappNotiHandler_` ส่ง `view` กลับ → `NotiBell` เรียก
  `onNavigate(it.tab, it.focus, it.view)` → app.jsx **`dmjRequestFocus` + `dmjRequestView`
  ก่อน `handleSetTab`** → `useViewIntent("orders", ...)` ที่มีอยู่แล้วรับต่อ (ตัวเดียวกับชิปหน้าหลัก)
  · ⚠️ **ต้องเรียกทั้งคู่แม้ค่าจะว่าง** — ทั้งสองตัวล้างคำขอค้างของการกดครั้งก่อนให้ด้วย
    ไม่เรียก = กดแจ้งเตือนออเดอร์ใหม่แล้วตัวกรองเปลี่ยนเองจากคำขอเก่า
- ⚠️ **ปลายทางตัดสินตอน "อ่าน" ไม่ใช่ตอน "เขียน"** — `inappNotiRoute_(type, tab, view)`
  แปลงแถวเก่า (`type='shipment'`, `tab='stock'`, ไม่มี view) เป็น `orders`/`shipped` ให้ตอนส่ง
  ออกไป · แถวที่เขียนไปแล้วแก้ไม่ได้และอยู่ในชีตอีก 14 วัน — ไม่กู้ = ของที่ค้างอยู่ตอนนี้
  ยังพาไปผิดหน้าต่อไปอีกสองสัปดาห์
  · ⚠️ **เงื่อนไขต้องแคบ** — `type='shipment'` ยังถูกใช้กับ "รับของไม่ครบ" (`tab='tracking'`)
    ซึ่งพาไปถูกอยู่แล้ว เหมารวมทุกแถวที่ type ตรง = แก้ที่หนึ่งพังอีกที่โดยไม่มี error ให้เห็น
- เทสต์: `tests/inapp-noti.test.js` (eval `inappNotiRoute_` จริงจาก `.gs` ไม่ copy + meta-test
  คุมสายส่งทั้งเส้น และบังคับว่า **ทุก `view` ที่ GAS สั่งมาต้องมี `useViewIntent` รับจริง**
  รวมปลายทางที่ `inappNotiRoute_` กู้ให้แถวเก่าด้วย — ตั้งคำขอแล้วไม่มีใครอ่าน = ตายเงียบ)
  · browser test กดแจ้งเตือนของโอนจริงแล้วเช็คว่า **ตัวกรองที่ active คือ "🚚 ส่งแล้ว"
  และเลขที่ใบโอนขึ้นบนจอ** (เช็คแค่ว่า "อยู่แท็บ orders" ไม่พอ — เปิดผิดตัวกรองก็ยังผ่าน)

### แจ้งเตือนสำหรับหน้าร้าน/เซล + ของค้างรับ (ส.ค. 2026)

ก่อนหน้านี้ **`saler`/`storedevice` ไม่ได้รับแจ้งเตือนอะไรเลยสักอัน** และ `frontstore` ได้อันเดียว
— ทั้งที่เป็นคนที่อยู่หน้างานขายจริง · 2 ตัวนี้เติมช่องว่างนั้น (เจ้าของเลือกเอง ส.ค. 2026)

**1. "🛒 ของหมดหน้าร้าน — คลังยังมี สั่งได้เลย"** (`fsNeedsRestock_` ใน `syncZortBoth`)
- ตัวสแกนสต็อกใกล้หมดเดิมอ่าน**แค่คอลัมน์ H (คลัง)** ไม่เคยแตะ **G (หน้าร้าน)** เลย →
  คนขายรู้ตอนเดินไปหยิบแล้วไม่เจอเท่านั้น ทั้งที่**แก้ได้เดี๋ยวนั้นด้วยการกดสั่ง**
  (ต่างจากคลังหมดที่ต้องรอ PO — อันนั้นแจ้ง owner/warehouse อยู่แล้ว)
- ⚠️ **`FS_REORDER_MAX = 12` ต้องตรงกับ `needsReorder` ใน CategoryView เป๊ะ** — แจ้งเตือน
  พาไปหยุดที่ตัวกรอง "🛒 ควรสั่ง" ถ้าเกณฑ์สองฝั่งไม่ตรง = แจ้งของที่ตัวกรองปลายทางกรองทิ้ง
  → กดตามไปแล้วไม่เจอสิ่งที่แจ้งเตือนพูดถึง **เงียบสนิท** (มีเทสต์เทียบเลขสองไฟล์)
- ⚠️ **default แจ้งเฉพาะที่หมดเกลี้ยง (`FRONTSTORE_OOS_ALERT_MAX` = 0)** — ตัวกรอง "ควรสั่ง"
  มีเป็นสิบ-ร้อยรายการแทบทุกวันและเลขแทบไม่ขยับ ยิงทั้งกองทุกวัน = **กระดิ่งกลายเป็นวอลเปเปอร์
  ที่ไม่มีใครอ่าน** (ซึ่งทำให้แจ้งเตือนตัวอื่นที่สำคัญกว่าถูกกลบไปด้วย) · ปรับขึ้นได้ที่ Script
  Property แต่ถูก clamp ที่ `FS_REORDER_MAX` เสมอ (เกินนั้นปลายทางไม่โชว์)
- ⚠️ **audience ไม่มี `warehouse` โดยตั้งใจ** — คนคลังไม่ได้กดสั่งแทนหน้าร้าน และจะได้
  "ออเดอร์ใหม่" อยู่แล้วตอนหน้าร้านกดสั่งจริง · ใส่เข้าไป = แจ้ง 2 เด้งเรื่องเดียวกัน
- **`useViewIntent("categories", ...)` ต้องล้างตัวกรองที่ "ซ่อนของ" ก่อนเสมอ** (หมวดที่เลือกไว้ /
  โหมด ⭐ ของฉัน / คำค้น / สี / ร้าน) — คนที่ค้างไว้ที่หมวดใดหมวดหนึ่งจะเห็นจอว่างเปล่าทั้งที่
  ของอยู่ครบ แล้วแยกไม่ออกจาก "แอปพัง" (หลักเดียวกับที่ `useSkuFocus` ของแท็บ orders ทำ)
- เกาะการอ่านชีตรอบเดิมของ low-stock scan **ไม่เปิดชีตเพิ่ม** (รันทุก 2 ชม.)

**2. "⏳ ของค้างรับ N รายการ"** (`shipPendingAging_` + `notifyPendingReceives_`)
- หน้าติดตามมีป้าย "⏳ ค้าง N วัน" อยู่แล้ว **แต่ต้องเปิดหน้าไปดูเองถึงเห็น** — เคส
  TF-202608035 ที่หายไปโดยไม่มีใครทันสังเกตคือเคสนี้เป๊ะ · ตัวนี้เปลี่ยน "ต้องเปิดไปดูถึงจะรู้"
  เป็น "มันมาบอกเอง"
- ⚠️ **`SHIP_PENDING_ALERT_DAYS = 3` ต้องตรงกับป้ายแดงในหน้าติดตาม** (`age >= 3`) —
  สองที่พูดคนละเลข = คนอ่านไม่รู้ว่าอันไหนคือเกณฑ์จริง (มีเทสต์เทียบสองไฟล์)
- ⚠️ **ยิง 2 แถวแยกกัน ไม่ใช่แถวเดียวส่งทุก role** — หน้าร้าน/employee = คน **กดรับ** ได้ →
  พาไป `orders`+`shipped` · คลัง/เจ้าของ = กดรับแทนไม่ได้ ต้อง **ตามของ** → พาไป `tracking`
  · ยุบเป็นแถวเดียว = อีกฝั่งได้ปลายทางที่ตัวเองทำอะไรไม่ได้ (บั๊กเดียวกับที่เพิ่งแก้ไป)
  · **dedupKey ต้องคนละคีย์** ไม่งั้นตัวที่ยิงทีหลังถูก dedup ทิ้งเงียบ ๆ = ฝั่งนั้นไม่ได้รับเลย
- ⚠️ **ต้องเรียก "ก่อน" ทางออกลัด `if (!toArchive.length) return;`** — วันที่ไม่มีอะไรต้อง archive
  = ไม่มีใครกดรับเลย = **วันที่ของค้างเยอะที่สุดพอดี** วางไว้ท้ายสุด = เงียบในวันที่ควรเตือนที่สุด
- ใช้ `parseShipDayMs_` (รองรับปี พ.ศ. — บทเรียนข้อ 11) · อ่านวันที่ไม่ออก/อนาคต/เก่าเกิน
  400 วัน → **ข้าม ไม่เดาอายุ** (หลักเดียวกับ `trackAgeDays` ฝั่งเว็บ)
- เกาะแถวที่ `archiveReceivedShipments` อ่านมาแล้ว ไม่อ่านชีตซ้ำ · ห่อ try/catch (แจ้งเตือน
  พลาดห้ามขวางงาน archive)

เทสต์: `tests/restock-pending-noti.test.js` (27 เคส — eval ฟังก์ชันจริงจาก `.gs` ไม่ copy)
คุมทั้งสูตรและจุดเชื่อมต่อ: เกณฑ์ตรงกับ UI ทั้ง 2 คู่, ทุก role ใน audience มีแท็บปลายทางจริง
(ไม่มี = `NotiBell` return เงียบ ๆ กดไม่ติดโดยไม่มี error), ลำดับก่อน early-return, ไม่อ่านชีตซ้ำ
· browser test กดแจ้งเตือนจริงแล้วเช็คว่า **ตัวกรอง "🛒 ควรสั่ง" ถูกเปิด (`data-reorder="on"`)
และลิสต์ถูกกรองจริง** (เห็น FLW002 · ไม่เห็น VAS001) — เช็คแค่ "มาถึงแท็บ" ไม่พอ

**ยังไม่ทำ (เจ้าของยังไม่สั่ง)**: มอบหมายงาน MTO → ยิงเจาะตัวคนที่ถูกมอบหมาย (`assignMtoJob`
· `audience: "staff:STxxxx"` เขียนไว้ครบและมีเทสต์แล้ว **แต่ยังไม่มีใครเรียกใช้เลยสักที่**) ·
ใบเสนอราคาใกล้หมดอายุ → saler (ต้องตั้ง trigger ใหม่ + ยิง ZORT วนหลายหน้า แพงกว่าอันอื่นมาก) ·
ของที่ตัวเองกดดาว ⭐ ใกล้หมด → ยิงเจาะตัว (ต่อยอดจากตัวสแกนข้อ 1 ได้เลย)
...
### 💰 ยอดขายเป็นเงินต่อเซล (ส.ค. 2026)

เดิม staffperf นับ "ออกบิลขาย" เป็น **จำนวนใบ** เท่านั้น (`byCat.sale`) → เจ้าของเห็น "คนนี้ 40 ใบ
คนนั้น 45 ใบ" แต่ตอบ **"เซลคนไหนทำเงินเข้าร้านมากสุด"** (คำถามบริหารคนอันดับ 1 ของธุรกิจขาย) ไม่ได้
· ยอดบาทมีในชีต "บิลขาย" (col I=ยอดสุทธิ) ครบทุกแถวอยู่แล้ว — แค่ไม่เคยมีใครหยิบมารวม

- **`staffPerfAggregateSales_(rows, monthKey)`** (pure, เทสต์ eval ตรงจาก `.gs`) — รวม `{revenue, bills}`
  ต่อผู้ขาย · อ่าน **จากชีตบิลขายโดยตรง** (แหล่งเงินจริง) ไม่แกะจาก audit detail ที่เป็น JSON string เปราะ
- **ผูก staffId ด้วย `nameToId` ชุดเดียวกับ audit** — ชื่อผู้ขายในชีตบิลขาย (col F) เป็น
  "ชื่อ (ตำแหน่ง)" รูปแบบเดียวกับ actor · ⚠️ **ห้ามสร้าง map จับคู่ชื่อชุดใหม่** จะ drift กับ audit
- อ่านชีตบิลขาย **แค่ A..I (9 คอลัมน์)** — ชีตนี้เล็ก (1 แถว/บิล) แต่ยังไม่ getDataRange ทั้งชีต ·
  กรองเฉพาะสถานะ "สำเร็จ" (เผื่ออนาคตมีบิลยกเลิก) แต่ถ้าอ่านไม่ถึง col สถานะก็ยังนับ (migration-safe)
- แถวต่อคนมี `saleRevenue`/`saleBills` · คนที่**ขายได้อย่างเดียว** (ไม่มีงาน audit อื่น) ไม่ถูก filter
  ทิ้ง (เพิ่ม `|| row.saleRevenue > 0`) · UI โชว์ 💰 บนการ์ด + กล่อง "ยอดขาย/บิล/เฉลี่ยต่อบิล" ตอนกาง
  + ยอดขายรวมต่อกลุ่มตำแหน่ง
- ⚠️ **`fmtBfull` เท่านั้น (global จาก ui.jsx)** ไม่ใช้ `money`/`fmtB` ในนี้ — ต้องเห็นยอดเต็มบาท
- เทสต์: `tests/staff-perf.test.js` (`staffPerfAggregateSales_` + meta-test ว่า `staffPerfBuild_`
  อ่านชีตบิลขาย/ผูกด้วย nameToId/มี saleRevenue ในแถว + UI โชว์เป็นเงิน) · browser test มีเซล
  `มาลี ขายเก่ง` ยอด `฿87,500` ยืนยันบนจอจริงว่าโชว์เงิน ไม่ใช่แค่จำนวนใบ
...
**ต่อยอดได้ (ยังไม่ทำ)** — เรียงตามความคุ้ม (audit มุมเจ้าของ ส.ค. 2026):
1. **แยก "ขาด" กับ "ลา/วันหยุด"** — ตอนนี้ระบบ**ไม่มีคอนเซ็ปต์ลา/วันหยุดเลย** · `daysAbsent`
   (`appsscript_complete.gs` 3 ที่: staffperf/สรุปวันนี้/สรุปเดือน) นับ "วันในอดีตที่ role นั้นมีกะแต่
   ไม่กดเข้า" → **ลาป่วยที่อนุมัติแล้ว + วันหยุดร้านที่ตั้งกะไว้ตรงวัน = ถูกนับเป็นขาดทั้งหมด** →
   เลข "ขาด" เชื่อไม่ได้สำหรับตัดสินวินัยการเข้างาน · แก้: เพิ่ม event ชนิด "ลา" ที่ `fixAttendance`
   ใส่ย้อนหลังได้ + `attSummarize_` เห็นวันนั้นเป็นลาไม่ใช่ขาด + Script Property วันหยุดร้าน ·
   **แตะ semantics การลงเวลาที่ใกล้ข้อมูลเงินเดือน — ทำเป็นก้อนแยก อย่าบันเดิลกับงานอื่น**
2. **แนวโน้มคนคนเดียวข้ามเดือน** — แถบเหลืองบอกเองว่า "ใช้ดูแนวโน้มข้ามเดือน" แต่จอโหลดทีละ
   เดือน ไม่มีเส้น/sparkline หลายเดือน · เจ้าของต้องกด dropdown สลับแล้วจำเอาเอง
3. **เฉลี่ยนาที/ออเดอร์** — จับเวลาแต่ละขั้น (เริ่ม→จบจัดออเดอร์) · **ติดกำแพงข้อมูลจริง**: Audit Log
   มีแต่ "เมื่อไหร่" ไม่มี "ใช้เวลาเท่าไหร่" → ต้องเริ่มบันทึก timestamp เริ่มงานใหม่ ไม่ใช่คำนวณจากของเดิม
