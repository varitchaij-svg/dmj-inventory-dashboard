...
### ⭐ saler นับหน้าร้านได้ แต่ **ไม่บังคับ** (ส.ค. 2026)

เจ้าของสั่ง: "เพิ่มปุ่มกดเช็คสินค้าให้ตำแหน่ง saler" — saler ยืนอยู่หน้าร้านเห็นชั้นวางจริง
ควรนับ/บันทึกยอดได้เหมือนหน้าร้าน **แต่ห้ามบังคับ** เพราะงานหลักคือปิดการขายให้ทันลูกค้าที่
ยืนรออยู่ตรงหน้า — บังคับนับก่อนสั่งเมื่อไหร่ = เอางานคนอื่นไปขวางการขาย

- **ธงเดียวเดิม (`needFsCheck`) ถูกแยกเป็น 2 ตัว** ใน `OrderModal` — ของเดิมทำหน้าที่ทั้ง
  "โชว์การ์ดนับ" และ "กั้นปุ่มยืนยัน" พร้อมกัน เปิดให้ saler เมื่อไหร่ก็โดนกั้นไปด้วยทันที
  · **`canFsCheck`** (frontstore/employee/**saler**) = การ์ดนับโผล่ + บันทึกยอดได้
  · **`mustFsCheck`** (frontstore/employee) = กั้นปุ่มยืนยันจนกว่าจะกรอก (`fsBlocked`)
- ⚠️ **ยุบกลับเป็นตัวเดียวเมื่อไหร่ saler กดสั่งของไม่ได้ทั้งตำแหน่ง โดยไม่มีอะไรเตือน**
...
- ⚠️ **กรอบสีเตือน (ส้ม) ผูกกับ `fsBlocked` ไม่ใช่ "ยังไม่กรอก"** — ไม่งั้น saler เห็นกรอบส้ม
...
- **ไม่ต้องแตะ `.gs` เลย** — `updateFrontStore` อยู่ใน `COMMON_ACTIONS_` อยู่แล้ว saler จึงมีสิทธิ์
  ฝั่ง server มาตั้งแต่ต้น (เป็นแค่การ *เปิดให้เห็น* ไม่ใช่การให้สิทธิ์ใหม่) → **ไม่ต้อง deploy GAS**
- **`storedevice` ยังไม่เปิด** — เจ้าของขอมาเฉพาะ saler · จะเปิดต้องแก้ **ทั้ง 2 ที่** พร้อมกัน
  แล้วอัปเดต `tests/saler-fs-count.test.js`
- เทสต์: `tests/saler-fs-count.test.js` (21 เคส — **eval นิพจน์ role จริงจาก `.jsx` ไม่ copy**
  เหมือน auth.test.js · คุมทั้ง drift ของ 2 จุด และ `fsBlocked`/`fsDirty` ผูกกับธงถูกตัว)
  + browser test 2 เคส (`นับหน้าร้าน (saler)` / `(frontstore)`) ที่ยืนยันบนเบราว์เซอร์จริงว่า
  **saler เห็นการ์ดนับแล้วยังกดยืนยันสั่งได้เลย และหน้าร้านยังถูกบังคับเหมือนเดิม** — ทดสอบคู่กัน
  โดยตั้งใจ เพราะปลดกั้นให้ saler แล้วเผลอปลดของหน้าร้านไปด้วยเป็นความพังที่เงียบสนิท
